...
# We declare a typical parameter T with a restriction that this is the heir to BASE
...
class BaseDAO(Generic[T]):
    ...
    async def upsert(cls, session: AsyncSession, unique_fields: List[str], values: BaseModel):
        """Create an entry or update the existing"""
        values_dict = values.model_dump(exclude_unset=True)
