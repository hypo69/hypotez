...
from pydub import AudioSegment  # Library for conversion audio
from gtts import gTTS  # Library for text reproduction
from .chatterbox import *

# We indicate the path to ffmpeg
...
async def text_to_speech_and_play(text, channel):
    ...
    tts = gTTS(text=text, lang='ru')  # Replace 'ru' with the desired tongue
    audio_file_path = f"{tempfile.gettempdir()}/response.mp3"  # The path to the temporarily created file
    tts.save(audio_file_path)  # We save the audio file

    voice_channel = channel.guild.voice_client
    if not voice_channel:
        voice_channel = await channel.connect()  # We connect to the voice channel

    voice_channel.play(discord.FFmpegPCMAudio(audio_file_path), after=lambda ex: logger.info(f'Finished playing: {ex}'))

    while voice_channel.is_playing():  # Waiting while the sound plays
        await asyncio.sleep(1)

    await voice_channel.disconnect()  # Turn off
...
async def on_message(message):
    ...
    if message.author == bot.user:
        return  # We ignore messages from ourselves

    if message.content.startswith(PREFIX):
        await bot.process_commands(message)
        return  # We process commands
    ...
    if message.author.voice:
        # If the user is in the voice channel, we connect and play the answer
        await text_to_speech_and_play(response, message.author.voice.channel)
    else:
        await message.channel.send(response)  # We send the answer to the text channel
