...
N_MELS = 40  # Should coincide with the dimension of the entrance to the model
SR = 16000  # Sampling frequency


def extract_melspectrogram(file_path, sr=SR, n_mels=N_MELS):
    y, _ = librosa.load(file_path, sr=sr)
    y, _ = librosa.effects.trim(y)  # Remove the silence at the beginning and end

    if len(y) == 0:
        return None  # Passing empty files

    mel_spec = librosa.feature.melspectrogram(y=y, sr=sr, n_mels=n_mels)
    mel_db = librosa.power_to_db(mel_spec, ref=np.max)  # We convert to DB

    feature = np.mean(mel_db, axis=1)  # We averaged time (by columns)
...
for file in os.listdir(AUDIO_PATH):
    if file.endswith(".wav"):
        name = file.split("_")[0]  # We get the name of a friend
