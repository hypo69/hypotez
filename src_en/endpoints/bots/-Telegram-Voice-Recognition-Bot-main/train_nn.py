...
N_MELS = 40  # The number of small banks


# Augmentation function
def augment_audio(y, sr):
    if np.random.rand() < 0.5:
        y = y + 0.005 * np.random.randn(len(y))  # Adding noise
    if np.random.rand() < 0.5:
        shift = np.random.randint(sr // 10)  # Shift up to 0.1 sec
        y = np.roll(y, shift)
    return y


# The function of extracting signs
def extract_melspectrogram(file_path, sr=SR, n_mels=N_MELS, augment=False):
    ...
    return np.mean(mel_db, axis=1)  # Aviation in time
...
for file in os.listdir(AUDIO_PATH):
    if file.endswith(".wav"):
        ...
        aug_features = extract_melspectrogram(file_path, augment=True)  # Augmented data
...
optimizer = optim.Adam(model.parameters(), lr=0.001, weight_decay=1e-4)  # L2-regulator
