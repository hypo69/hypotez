...
"""Module for controlling and processing images, as well as promotion on Facebook and Prestashop. Refers to the store `emil-design.com`
====================================
Basic possibilities:
    - <Instructions for the Gemini model: Description of images using gemini ai.>
    - <Instructions for the Gemini model: downloading the descriptions of goods in Prestashop.>
    - ..... <next, if you are>
Classes:
    `Config` - <Instruction for the Gemini model: Give a full description and purpose of this class>
    `Emildesign` - <Instruction for the model: give a full description and purpose of this class>
...
class Config:
    ...
    system_instruction:str = ' ' # <- This is a gap!
...
class EmilDesign:
    ...
    def describe_images(
        ...
        try:
            ...
            for img in images_to_process:
                ...
                time.sleep(15)  # Delay between requests
    ...
    def upload_described_products_to_prestashop(
        ...
        Args:
            ...
            Usually I assign languages in this order 1 - en; 2 - he; 3 - ru. 
            It is important to check the order of the Yakykov target database.
            Here is a sample code to get languages from a particular database.
            >> Import Language
            >> Lang_class = Prestalanguage ()
            >> Print (Lang_class.get_languages_schem ())
        ...
        RAISES:
            Filenotfounderror: if locals file is not found.
            Exception: If Arror OCCURS DURING PRESTASHOP UPLOAD."""
        try:
            ...
            """Important! When loading goods in Prestashop, it is necessary to specify the language in which the names and characteristics of the goods will be displayed.
            In this case, the default language is Hebrew (he = 2), but you can also specify English (EN) or Russian (ru)
            Indexes can change depending on the settings of the store. Usually I expose the `1` index for English,` 2` for Hebrew and `3` for Russian.
            Table with indexes for` emil-design.com` is in the `locals.json` file in the folder` shop_locales`"""
