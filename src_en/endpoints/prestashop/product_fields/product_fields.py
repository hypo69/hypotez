...
============================
The Product_fields module is designed to work with products in Prestashop. 
It provides a class Productfields, which allows you to conveniently control the attributes of the goods, both main and multi -language.
Each product field for tables Presashop is scheduled
    
    List of fields: https://github.com/hypo69/hypotez/blob/master/src/endpoints/prestashop/product_fields/fields_list.txt
    Default values: https://github.com/hypo69/hypotez/blob/master/src/endpoints/prestashop/product_fields/product_fields_default_values.json
    Documentation: https://github.com/hypo69/hypotez/blob/master/docs/ru/src/endpoints/prestashop/product_fields/product_fields.py.py.py.md
...
@dataclass
class ProductFields:
    """A class describing the fields of the product in the API Prestashop format.
     Language indices that I install in the Preshop database:
    1. English
    2. Hebrew
    3. Russian"""
    ...
    def _payload(self,*args, **kwargs) -> bool:
        """The function uploads the default field values and overwrites them with the values from Kwargs.
        ...
        try:
            for name, value in data_dict.items():
                setattr(self.presta_fields, name, value ) # <- setting the default field values

        except Exception as ex:
            logger.error(f"Exception ", ex)
            return False 
    
        # All the transmitted Kwargs arguments are set in the values of the fields.
    ...
    def _set_multilang_value(self, field_name: str, value: str, id_lang: Optional[int | str] = 1) -> bool:
        """Sets a multi -language value for a given field.

        Args:
            Field_name (str): Field name (for example, 'Name', 'Description').
            Value (str): meaning for installation.
            ID_LANG (Optional [Union [int, str]]): ID of the tongue. If not specified, Self.id_lan is used.

        Description:
            The function sets multi -language value for the specified field of the object.  
            The field can store values for different languages.  Values are stored in the form of a list of dictionaries,
            where each dictionary is a meaning for a particular language and has a structure:

            {'attrs': {'id': 'Language_id'}, 'Value': 'Language_value'}
             {'Id': 'Language_id'}, 'Value': 'Language_value'}

            - 'Attrs': a dictionary containing attributes of the value.  In this case, an obligatory attribute is 'id',
                       which is the language identifier.
            - 'Value': The value of the field for the specified language.

            If the field with the specified name does not exist, it is created. If the field exists, but does not have
            The expected structure (a dictionary with the key 'Language' containing a list), the field is rewritten.
            If the field exists and has the correct structure, the function tries to update the value for
            the specified language. If the language already exists in the list, its value is updated. If the language
            It does not exist, a new entry is added to the list.

        Returns:
            Bool: True, if the value is successfully set, FALSE in case of an error."""
        id_lang = self.id_lang if not id_lang else id_lang
        def escape_and_strip(text: str) -> str:
            """Cleans and shields the line, replacing the characters "'' '' 'with" \' "and '\"',
            Removing gaps at the beginning and end."""
            if not text:
                return ''
            try:
                # Shinting "''" "" ", replacement"; "on" <br> ", removal of extra spaces
        ...
        try:
            ...
            if field is None:
                ...
                if not isinstance(field, dict) or 'language' not in field or not isinstance(field['language'], list):
                    # If the field is not a dictionary with a key 'Language' containing a list, then the creation of a dictionary
                    setattr(self.presta_fields, field_name, {'language': lang_data})
                else:
            ...
            logger.error(f"""Error setting the value in the multi -language field {Field_name}
            Meaning {value}: \ n {ex}""")  # Include exception details in the log
            return False
    ...
    # None
    # Fields of the PS_PRODUCT table
    # None
    ...
    def id_product(self, value: int = None):
        """Setter `ID` goods. For the new product, the ID is prescribed from `Prestashop`."""
        try:
    ...
    def id_category_default(self, value: int):
        """Setter `ID` of the main category of goods."""
        try:
    ...
    def position_in_category(self, value:int = None):
        """Setter Product position in the category."""
        try:
    ...
    def minimal_quantity(self, value: int = 1):
        """Setter is the minimum amount of goods for ordering."""
    ...
    def low_stock_threshold(self, value: int):
        """Setter threshold value for notification of low reserve."""
    ...
    def low_stock_alert(self, value: int):
        """Setter flag of low reserve notification."""
    ...
    def unit_price_ratio(self, value: float):
        """Setter ratio of price per unit."""
    ...
    def additional_shipping_cost(self, value: float):
        """Setter additional delivery cost."""
    ...
    def location(self, value: str):
        """Setter Location of the goods in the warehouse."""
    ...
    def out_of_stock(self, value: int = None):
        """Setter action in the absence of goods in stock."""
    ...
    def customizable(self, value: int):
        """Setter flag of customization capabilities."""
    ...
    def uploadable_files(self, value: int):
        """setter flag of file downloading."""
    ...
    def active(self, value: int = 1):
        """Setter Flag of Product activity."""
        self.presta_fields.active = value

    class EnumRedirect(Enum):
        """Listing for types of redirects."""
        ERROR_404 = '404'
    ...
    def id_type_redirected(self, value: int):
        """Setter ID of a connected redirect."""
    ...
    def available_for_order(self, value: int):
        """Setter accessibility flag for ordering."""
    ...
    def show_condition(self, value: int = 1):
        """Setter flag display of the condition of the goods."""
    ...
    def indexed(self, value: int = 1):
        """Setter flag of goods indexation."""
        self.presta_fields.indexed = value

    class EnumVisibity(Enum):
        """Listing for the visibility of the goods."""
        BOTH = 'both'
        CATALOG = 'catalog'
        SEARCH = 'search'
        NONE = 'none'
    ...
    def cache_is_pack(self, value: int = 1):
         """Setter Caching Flag as a package of goods."""
    ...
    def cache_has_attachments(self, value: int = 1):
         """Setter Flag of Investing Caching."""
    ...
    def cache_default_attribute(self, value: int = 1):
        """Setter ID of the default attribute for caching."""
    ...
    def date_upd(self, value: datetime = datetime.now()):
         """Setter date for updating goods."""
    ...
    def advanced_stock_management(self, value: int):
         """Setter flag of expanded stock management."""
    ...
    def pack_stock_type(self, value: int):
        """Setter type of control of the package of goods."""
    ...
    def product_type(self, value: EnumProductType | str = EnumProductType.STANDARD):
        """Setter type of product."""
        self.presta_fields.product_type = str(value)


    
    # None
    # Fields of the PS_PRODUCT_LANG table
    # None
    ...
    def name(self, value: str):
        """Setter Product name. Multi -language field."""
    ...
    def description(self, value: str):
        """Setter Description of the goods. Multi -language field."""
    ...
    def description_short(self, value: str):
        """Setter brief description of the goods. Multi -language field."""
    ...
    def link_rewrite(self, value: str):
        """Setter URL of goods. Multi -language field."""
    ...
    def meta_description(self, value: str):
        """Setter Meta Description of the product. Multi -language field."""
    ...
    def meta_keywords(self, value: str):
        """Setter meta keywords of the product. Multi -language field."""
    ...
    def meta_title(self, value: str):
        """Setter Meta product headline. Multi -language field."""
    ...
    def available_now(self, value: str):
        """setter text "in stock". Multi -language field."""
    ...
    def available_later(self, value: str):
        """Setter text "expected". Multi -language field."""
    ...
    def delivery_in_stock(self, value: str):
        """Setter delivery text if there are. Multi -language field."""
    ...
    def delivery_out_stock(self, value: str):
        """Setter delivery text in the absence. Multi -language field."""
    ...
    def delivery_additional_message(self, value: str):
        """Setter Additional delivery message. Multi -language field."""
    ...
    def affiliate_short_link(self, value: str):
        """Setter short affiliat link. Multi -language field."""
    ...
    def affiliate_text(self, value: str):
        """Setter text of the affiliat. Multi -language field."""
    ...
    def affiliate_summary(self, value: str):
        """Setter brief description of the affiliat. Multi -language field."""
    ...
    def affiliate_summary_2(self, value: str):
        """Setter is a second brief description of the affiliat. Multi -language field."""
    ...
    def affiliate_image_small(self, value: str):
        """Setter is a small image of an affiliate. Multi -language field."""
    ...
    def affiliate_image_medium(self, value: str):
        """Setter Affiliat's average image. Multi -language field."""
    ...
    def affiliate_image_large(self, value: str):
        """Setter is a large image of the affiliat. Multi -language field."""
    ...
    def ingredients(self, value: str):
        """Setter list of ingredients. Multi -language field."""
    ...
    def specification(self, value: str):
        """Setter specification of goods. Multi -language field."""
    ...
    def how_to_use(self, value: str):
        """Setter how to use the product. Multi -language field."""
    ...
    def _ensure_associations(self):
        """It is convinced that the structure of the Associations exists in Presta_fields."""
    ...
    def additional_category_append(self, category_id: int | str):
        """Adds a connection with the category if it is not yet."""
        ...
        # There is already such a category.
    ...
    def additional_categories_clear(self):
        """Cleans all connections with categories."""
        self._ensure_associations()
    ...
    def product_image_append(self, image_id: int):
        """Adds a connection with the image."""
        self._ensure_associations()
    ...
    def product_images_clear(self):
        """Cleans all connections with images."""
        self._ensure_associations()
    ...
    def images_urls(self) -> Optional[List[str]]:
        """Property URL list of additional images."""
    ...
    def images_urls_append(self, value: List[str] = None):
        """Installs the URL list, where to download additional images."""
        if value is None:
            return
        ...
        for url in valid_urls:
            if url not in self.assist_fields_dict['images_urls']: # Checking for uniqueness
    ...
    def product_combination_append(self, combination_id: int):
        """Adds a connection with the combination."""
        self._ensure_associations()
    ...
    def product_combinations_clear(self):
        """Cleans all ties with combinations."""
        self._ensure_associations()
    ...
    def product_options_append(self, product_option_value_id: int):
        """Adds a connection with the value of the option of goods."""
        self._ensure_associations()
    ...
    def product_options_clear(self):
        """Cleans all connections with the values of the options."""
        self._ensure_associations()
    ...
    def product_features_append(self, feature_id: int, feature_value_id: int):
        """Adds a connection with the product characteristics."""
        self._ensure_associations()
    ...
    def product_features_clear(self):
        """Cleans all connections with the characteristics of the goods."""
        self._ensure_associations()
    ...
    def product_product_tags(self) -> Optional[List[dict]]:
        """Returns a list of tags for search engines"""
    ...
    def product_stock_available_append(self, stock_available_id: int, product_attribute_id: int):
        """Adds a connection with accessibility in the warehouse."""
        self._ensure_associations()
    ...
    def product_stock_availables_clear(self):
        """Cleans all connections with the availability in the warehouse."""
        self._ensure_associations()
    ...
    def product_attachment_append(self, attachment_id: int):
        """Adds a connection with the investment."""
        self._ensure_associations()
    ...
    def product_attachments_clear(self):
        """Cleans all ties with investments."""
        self._ensure_associations()
    ...
    def product_accessory_append(self, accessory_id: int):
        """Adds a connection with the accessory."""
        self._ensure_associations()
    ...
    def product_accessories_clear(self):
        """Cleans all ties with accessories."""
        self._ensure_associations()
    ...
    def product_bundle_append(self, bundle_id: int, product_attribute_id: int, quantity: int):
        """Adds a connection with the bank's bank."""
        self._ensure_associations()
    ...
    def product_bundle_clear(self):
        """Cleans all connections with the gas bands."""
        self._ensure_associations()
        if 'product_bundle' in self.presta_fields.associations:
            del self.presta_fields.associations['product_bundle']

    def to_dict(self) -> Dict[str, Any]:
        """Converts the Productfields object into a dictionary for Prestashop API,
        excluding the keys whose values are None or an empty line,
        And forms multi -language fields in the right format. All fields should be represented as lines.

        Returns:
            DICT [str, Any]: Dictionary with fields ready for Prestashop API."""
        product_dict = {}
        ...
        if self.id_tax_rules_group is not None:
            product_dict["id_tax_rules_group"] = str_val(self.id_tax_rules_group)
        ...
        # Adding associations, if they are
        associations_dict = {}
        if hasattr(self.presta_fields, 'associations') and self.presta_fields.associations:
            ...
            if associations_dict:  # Only if you have something to add, add the Associations key
                product_dict["associations"] = associations_dict

        return product_dict

    def _format_multilang_value(self, data: Any) -> List[Dict[str, str]]:
        """Formats multi -language values in the list of dictionaries for Prestashop API. All values are presented as lines.

        Args:
            Data (Any): Paul value. If this is a dictionary, the structure is expected {'Language': [{'attrs': {'id': Lang_id}, 'Value': Value}]}}

        Returns:
            List [dict [str, str]]: a list of dictionaries where each dictionary contains 'id' and 'value' (all as lines) for each language."""
        # result = []
