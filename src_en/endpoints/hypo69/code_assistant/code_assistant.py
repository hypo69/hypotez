
# # \file /src/endpoints/hypo69/code_assistant/code_assistant.py
# -*- coding: utf-8 -*-
# ! .pyenv/bin/python3
"""The Machine Learning Model Modul of the Comedical Learning Model, Documentation for the Project, Code and Test examples
======================================================================================ward

: Class: `Codeassistant`, reads code files, gives the code to the model, the model will process the code and returns it to the class, the class retains the result
In the Directory `doCs/gemini`, depending on the role, the files are stored in 

An example of use
-------------------

An example of using the class `codeassistant`:
# Ask the role of the performer, language
`` `python

    Assistant = Codeassistant (Role = 'Code_Checker', Lang = 'Ru', Model = ['Gemini'])
    Assistant.Process_Files ()
`` `

.. Header.py:
    `` `MERMAID
    Flowchart TD
        Start -> Header [<code> Header.py </fode> <br> Determine Project Root]
    
        Header -> Import [Import Global Settings: <br> <sode> from SRC Import GS </Code>] 
    `` `

[Documentation] (https://github.com/hypo69/hypotez/blob/master/docs/en/src/endpoints/hypo69/code_assistant/code_assistant.py.md)"""

import asyncio
import argparse
import sys
import os
from pathlib import Path
from typing import Iterator, List, Optional
from types import SimpleNamespace
import signal
import time
import re
import fnmatch

import header
from header import __root__
from src import gs
from src.utils.jjson import j_loads, j_loads_ns
from src.llm.gemini import GoogleGenerativeAi
from src.llm.openai import OpenAIModel
from src.utils.printer import pprint as print
from src.utils.path import get_relative_path
from src.logger.logger import logger
from src import USE_ENV

# -------------------------- file config.py ----------------------------------

class Config:
    ...
    ENDPOINT: Path = __root__ / 'src' / 'endpoints' / 'hypo69' / 'code_assistant'   
    config: SimpleNamespace = j_loads_ns(ENDPOINT / 'code_assistant.json')
    roles_list:list = config.roles
    languages_list:list = config.languages
    role: str = 'doc_writer_md'
    lang: str = 'ru'
    process_dirs:list[Path] = config.process_dirs
    exclude_dirs:list[Path] = config.exclude_dirs
    exclude_files_patterns:list[Path] = config.exclude_files_patterns
    include_files_patterns:list[Path] = config.include_files_patterns
    exclude_files:list[Path] = config.exclude_files
    exclude_dirs:list[Path] = config.exclude_dirs
    response_mime_type:str = config.response_mime_type 
    output_dir: str | Path = ''
    output_directory_patterns:list = config.output_dirs
    remove_prefixes:str = config.remove_prefixes
    model_name:str = config.model_name

    @classmethod
    @property
    def code_instruction(self):
        """Code_instruction - code instruction.
        For each call, it reads a file with the instructions, which allows you to update the instructions "on the fly""""
        try:
            return Path( Config.ENDPOINT / 'instructions'/ f'instruction_{Config.role}_{Config.lang}.md'
                ).read_text(encoding='UTF-8')

        except Exception as ex:
            return ''

    @classmethod
    @property
    def system_instruction(self):
        """Instructions for the model.
        For each call, reads a file with the instructions, which allows you to update the instructions "on the fly"""
        try:
            return Path(Config.ENDPOINT / 'instructions' / f'CODE_RULES.{Config.lang}.MD'
                                ).read_text(encoding='UTF-8')
        except Exception as ex:
            return ''


    gemini: SimpleNamespace = SimpleNamespace(**{
        'model_name': os.getenv('GEMINI_MODEL') if USE_ENV else config.model_name or None, 
        'api_key': os.getenv('GEMINI_API_KEY') if USE_ENV else gs.credentials.gemini.onela.api_key or None, 
        'response_mime_type': 'text/plain',
    })

# -------------------------- end config.py---------------------------------------------------



class CodeAssistant:
    """Class for the work of the assistant programmer with AI models"""

    role: str = ''
    lang: str = ''
    system_instruction:str = ''
    code_instruction:str = ''
    gemini: 'GoogleGenerativeAi'
    openai: 'OpenAIModel'

    def __init__(
        self,
        role: Optional[str] = 'doc_writer_md',
        lang: Optional[str] = 'en',
        model_name:Optional[str] = '',
        system_instruction: Optional[str | Path] = None,
        **kwargs,
    ) -> None:
        """Initialization of the assistant with specified parameters.

        Args:
            ROLE (STR): role for the task.
            Lang (str): the language of execution.
            Models_list (List [str]): List of models for initialization.
            System_instruction (Str | Path): General instruction for the model. 
            ** kwargs: additional arguments for initializing models."""
        self.role = role or Config.role
        self.lang = lang or Config.lang
        self.system_instruction = system_instruction or Config.system_instruction

        filtered_kwargs = {
                    k: v
                    for k, v in kwargs.items()
                    if k not in ('model_name', 'api_key', 'generation_config', 'system_instruction') # <- I leave only what the model needs
                }
        self.gemini = GoogleGenerativeAi(     
                model_name = model_name or Config.gemini.model_name,
                api_key=kwargs.get('api_key', Config.gemini.api_key),# Kwargs value is priority,
                system_instruction= system_instruction or Config.system_instruction,
                generation_config = {'response_mime_type': kwargs.get( 'response_mime_type',  Config.response_mime_type)},
                **filtered_kwargs,
            )
        ...


  

    def send_file(self, file_path: Path) -> Optional[str | None]:
        """Sending the file to the model.

        Args:
            File_path (Path): The absolute path to the file that needs to be sent.
            File_name (Optional [str]): File name for sending. If not indicated and 'src' is absent, the file name is used unchanged.

        Returns:
            Optional [str | None]: URL file, if successfully sent, otherwise None."""
        try:
            # Sending the file to the model
            response:str = self.gemini.upload_file(file_path)

            if response:
                if hasattr(response, 'url'):
                    return response.url

                ...
                return ''
        except Exception as ex:
            logger.error('Ошибка при отправке файла: ', ex)
            ...
            return ''
      
    # --- auxiliary functions ---

    def _yield_files_content(
        self,
        process_directory: str | Path,
    ) -> Iterator[tuple[Path, str]]:
        """Generates file ways and their contents according to these templates.

        Args:
            Process_Directory (Path | STR): Absolute Way to the Starting Directory

        Returns:
            Bool: Iterator"""

        process_directory: Path = process_directory if isinstance(process_directory, Path) else Path(process_directory)

        # Compilation of patterns of excluded files
        try:
            exclude_files_patterns = [
                re.compile(pattern) for pattern in Config.exclude_files_patterns
            ]

        except Exception as ex:
            logger.error(
                f'Не удалось скомпилировать регулярки из списка:/n{Config.exclude_files_patterns=}\n ', ex
            )
            ...

        # Iliteration by all files in the Directory
        for file_path in process_directory.rglob('*'):
            # Check for compliance with switching templates
            if not any(
                fnmatch.fnmatch(file_path.name, pattern) for pattern in Config.include_files_patterns
            ):
                continue

            # The references of the excluded directory
            if any(exclude_dir in file_path.parts for exclude_dir in Config.exclude_dirs):
                continue

            # Checking the excluded files according to the pattern
            if any(exclude.match(str(file_path.name)) for exclude in exclude_files_patterns):
                continue

            # Checking specific excluded files
            if str(file_path.name) in Config.exclude_files:
                continue

            # Reading the contents of the file
            try:
                content = file_path.read_text(encoding='utf-8')
                yield file_path, content
                # make_summary( docs_dir = start_dir.parent / 'docs' )  # <- ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~ DEBUG  (create `summary.md`)
            except Exception as ex:
                logger.error(f'Ошибка при чтении файла {file_path}', ex)
                ...
                yield None, None

            ...


    def _create_request(self, file_path: str, content: str, ) -> str:
        """Creating a request taking into account the role and language."""
        
        try:
            content_request: dict = {
                'role': self.role,
                'output_language': self.lang,
                f'file_location_in_project_hypotez': get_relative_path(file_path, 'hypotez'),
                'instruction': self.code_instruction or '',
                'input_code': f"""```python
                {content}
                ```""",
            } 
            return str(content_request)

        except Exception as ex:
            logger.error(f'Ошибка в составлении запроса ', ex, False)
            ...
            return str(content)


    def _remove_outer_quotes(self, response: str) -> str:
            """Removes external quotes at the beginning and at the end of the line, if they are present.

            Args:
                Response (str): The answer of the model that must be processed.

            Returns:
                STR: Cleaned content as a line.

            Example:
                >>> _remove_outer_quotes ('`` `md some content` ``')
                'sOME CONTENT'
                >>> _remove_outer_quotes
                'sOME CONTENT'
                >>> _remove_outer_quotes ('`` `Python Def Hello (): Print (" Hello ")` ``')
                '`` `Python Def Hello (): Print (" Hello ")` ``'"""
            try:
                response = response.strip()
            except Exception as ex:
                logger.error('Exception in `_remove_outer_quotes()`', ex, False)
                return ''

            # If the line begins with '`` `python' or '` `` Mermaid', returning it without changes. This is a suitable code
            if response.startswith(('```python', '```mermaid')):
                return response


            for prefix in Config.remove_prefixes:
                # Comparison with the prefix without registering register
                if response.lower().startswith(prefix.lower()):
                    # Remove the prefix and suffix "` `` "if it is
                    cleaned_response = response[len(prefix) :].strip()
                    if cleaned_response.endswith('```'):
                        cleaned_response = cleaned_response[: -len('```')].strip()
                    return cleaned_response

            # Return of the line unchanged if the conditions are not met
            return response

    async def _save_response(self, file_path: str|Path, output_dir: str|Path,  relative_to: str, response: str, model_name: str = 'gemini', ) -> bool:
        """Maintaining the answer of the model to the file with the addition of the suffix.

        The method retains the answer of the model to the file, adding to the current file extension
        Additional suffix, definition of the role.

        Args:
            File_path (Path): The initial path to the file in which the answer will be recorded.
            Response (str): The answer of the model that must be preserved.
            Model_name (str): the name of the model used to generate an answer.

        RAISES:
            Oserror: If you can’t create a directory or write to a file."""
        export_path:Path = None
        relative_path:Path = None
        try:
            # Obtaining a directory depending on the role
            _pattern:str = getattr(Config.output_directory_patterns, Config.role, '')

            for i, part in enumerate(file_path.parts):
                if part == relative_to:
                    relative_path = Path(*file_path.parts[i+1:])
                    break

            # Formation of the target directory taking into account the substitution of parameters <model> and <lang>
            target_dir: str = (
                f'docs/{_pattern}'
                .replace('<lang>', self.lang)
                .replace('<module>', str(relative_path))
                )



            # suffix depending on the role
            suffix_map = {
                'traslator':'',
                'code_checker': '.md',
                'doc_writer_md': '.md',
                'doc_writer_rst': '.rst',
                'doc_writer_html': '.html',
                'code_explainer_md': '.md',
                'code_explainer_html': '.html',
                'pytest': '.md',
            }
            suffix = suffix_map.get(self.role, '')

            # Export_path: Path = Path (output_dir / f '{target_dir} {sufix}') # Full way to the final documentation file
            export_path:Path = Path(output_dir / relative_path) # <- without creating additional subdorestories
            export_path.parent.mkdir(parents=True, exist_ok=True)

            try:
                export_path.write_text(response, encoding='utf-8')
                logger.success(f'Файл сохранён В:\n{export_path}')
                return True
            except Exception as e:
                logger.error(f'Файл не сохранился по адресу:\n{export_path}\nОшибка: {e}')
                return False


        except Exception as ex:
            logger.critical(f'Ошибка сохранения файла:\n{file_path=}', ex, False)
            ...
            return False

    # None

    async def process_files(
        self, 
        process_dirs: Optional[str | Path | list[str | Path]], 
        output_dir: Optional[str | Path | list[str | Path]],
        relative_to:str
    ) -> bool:
        """Compilation, sending a request and preserving the result.
        Args:
            Process_Dirs (str | path | list [str | path]): the path to the source files. The path should lead to the directory/pits, not to the file
            output_dir (str): the path of the final directory
            Relative_to (str): the name of the directory, from where the path to the file in `output_dir 'will be completed"""
        process_dirs = process_dirs if isinstance(process_dirs, list) else [process_dirs]
        for process_directory in process_dirs:

            process_directory:Path = Path(process_directory)

            if not process_directory.exists():
                logger.error(f"Директория не существует: {process_directory}")
                continue  # Transition to the next directory if the current

            if not process_directory.is_dir():
                logger.error(f"Это не директория: {process_directory}", None, False)
                continue  # Transition to the next directory, if the current is not a directory

            logger.info(f'Start {process_directory=}')

            for i, (file_path, content) in enumerate( self._yield_files_content(process_directory) ):
                if not any((file_path, content)):  # <- File reading error
                    logger.error(f'Ошибка чтения содержимого файла {file_path}\nContent {content} ')
                    continue

                if file_path and content:
                    logger.debug(f'Чтение файла номер: {i+1}\n{file_path}', None, False)
                    # In the case of a translator, additional parameters are not needed for a request
                    content_request:str = content if self.role == 'translator' else self._create_request(file_path, content)
                    response:str = await self.gemini.ask_async(content_request) 
                    # Response: str = await self.gemini.chat (content_request, 'coder') # <- chat (not checked)

                    if response:
                        response: str = self._remove_outer_quotes(response)
                        if Config.role == 'trainer': # Model training, the result is not soharanes
                            print(response, text_color = 'yellow')
                        elif not await self._save_response(file_path, output_dir, relative_to, response, ):
                                    logger.error(f'Файл {file_path} \n НЕ сохранился')
                                    ...
                                    continue
                    else:
                        logger.error('Ошибка ответа модели', None, False)
                        ...
                        continue

                await asyncio.sleep(20)  # <- ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~ DEBUG (change timeout)


  
    def run(self, start_from_file: int = 1) -> None:
        """Launching the process of processing files."""
        signal.signal(signal.SIGINT, self._signal_handler)  # Interruption processing (Ctrl+C)
        asyncio.run(self.process_files(start_from_file))

    def _signal_handler(self, signal, frame) -> None:
        """Transportation processing."""
        logger.debug('Процесс был прерван', text_color='red')
        sys.exit(0)


def parse_args() -> dict:
    """Analysis of command line arguments."""
    parser = argparse.ArgumentParser(description='Ассистент для программистов')
    parser.add_argument(
        '--role',
        type=str,
        default='code_checker',
        help='Роль для выполнения задачи',
    )
    parser.add_argument('--lang', type=str, default='ru', help='Язык выполнения')
    parser.add_argument(
        '--model',
        nargs='+',
        type=str,
        default=['gemini'],
        help='Список моделей для инициализации',
    )
    parser.add_argument(
        '--start-dirs',
        nargs='+',
        type=str,
        default=[],
        help='Список директорий для обработки',
    )
    parser.add_argument(
        '--start-file-number',
        type=int,
        default=1,
        help='С какого файла делать обработку. Полезно при сбоях',
    )
    return vars(parser.parse_args())


def main() -> None:
    """The function triggers an endless cycle in which files are processed taking into account the roles and languages indicated in the configuration.
    The configuration is updated in each cycle, which allows you to dynamically change the settings in the `code_assistant.json 'file during the program.
    For each combination of the language and the role, a class of class is created: Class: `Codeassistant`, which processes files using the given AI model."""


    # --- Settings for translator
    Config.roles_list = ['translator']
    Config.languages_list = ['he']
    Config.model_name = 'gemini-2.5-flash-preview-04-17'
    Config.process_dirs = [r'C:\Users\user\Documents\repos\public_repositories\algorithmica']
    Config.output_dir = r'C:\Users\user\Documents\repos\public_repositories\algorithmica-he'
    relative_to:str = 'algorithmica'
    Config.system_instruction = (Config.ENDPOINT / 'instructions' / f'DOCUMENT_TRANSLATOR.{Config.languages_list[0]}.MD').read_text(encoding='UTF-8')
    # None

    while True:
       
        # File processing for each combination of languages and roles
        for lang in Config.languages_list:
            
            for role in Config.roles_list:
                logger.debug(f'Start role: {role}, lang: {lang}', None, False)
                assistant_direct = CodeAssistant(
                    role=role,
                    lang=lang,
                    model_name = Config.model_name,
                    system_instruction = Config.system_instruction,
                )
                asyncio.run(
                    assistant_direct.process_files(Config.process_dirs, Config.output_dir, relative_to)
                    )


if __name__ == '__main__':
    main()