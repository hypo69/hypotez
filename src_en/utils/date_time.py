...
class TimeoutCheck:
    ...
    def get_input(self):
        """We request entering from the user."""
        self.user_input = input("U:> ")

    def input_with_timeout(self, timeout: int = 5) -> str | None:
        """We expect an input with a timeout.

        Args:
            Timeout (int): waiting time for entering in seconds.

        Returns:
            Str | None: entered data or None if there was a timeout."""
        # Run the flow to receive input from the user
        thread = threading.Thread(target=self.get_input)
        thread.start()

        # We expect the end of the flow or time out
        thread.join(timeout)

        if thread.is_alive():
            print(f"Timeout occurred after {timeout} seconds.")
            return  # None return if the timeout occurred
