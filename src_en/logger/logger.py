...
class JsonFormatter(logging.Formatter):
    ...
    def format(self, record):
        ...
        log_entry = {
            ...
            "message": record.getMessage().replace('"', "'", 1), # Corrected: only replace the first occurrence of "
...
class Logger(metaclass=SingletonMeta):
    ...
    def __init__(
        ...
        json_handler.setFormatter(JsonFormatter())  # Our custom formatter is used
        self.logger_file_json.addHandler(json_handler)

        # Remove all handlers that output to the console
