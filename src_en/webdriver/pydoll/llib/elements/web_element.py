## \file src/webdriver/pydoll/llib/elements/web_element.py
# -*- coding: utf-8 -*-
#! .pyenv/bin/python3

"""
This module provides a class for interacting with web elements.
==============================================================
...
```python
    ...
    async def main():
        # Assuming you have a connection_handler and object_id for a web element
        # handler = ConnectionHandler(connection_port=9222)
        # element = WebElement(object_id="some_object_id", connection_handler=handler)
        # await element.click()
        pass
...
:version: 1.0.0
:location: src/webdriver/pydoll/llib/elements/web_element.py
"""
...
from src.webdriver.pydoll.llib.commands import (
    DomCommands,
    InputCommands,
    PageCommands,
    RuntimeCommands,
...
from src.webdriver.pydoll.llib.protocol.dom.responses import (
    GetBoxModelResponse,
    GetOuterHTMLResponse,
)
from src.webdriver.pydoll.llib.protocol.dom.types import Quad
from src.webdriver.pydoll.llib.protocol.page.responses import CaptureScreenshotResponse
from src.webdriver.pydoll.llib.protocol.page.types import Viewport
from src.webdriver.pydoll.llib.utils import (
...
class WebElement(FindElementsMixin):  # noqa: PLR0904
    ...
    async def inner_html(self) -> str:
        ...
        This property retrieves the inner HTML content of the web element.
        It uses the `DomCommands.get_outer_html` CDP command to get the HTML markup
        of the element, which includes the element itself and its children.
    ...
    async def get_parent_element(self) -> 'WebElement':
        ...
        This method retrieves the parent element of the current web element.
        It executes a JavaScript script to get the parent node and then creates
        a new `WebElement` instance for the parent.

        Returns:
            WebElement: The parent web element.

        Raises:
            ElementNotFound: If the parent element is not found.
        """
    ...
    async def scroll_into_view(self):
        ...
        This method scrolls the web element into the visible area of the browser
        viewport if it is not already visible. It uses the `DomCommands.scroll_into_view_if_needed`
        CDP command to achieve this.
        """
    ...
    async def key_up(self, key: Key):
        ...
        This method dispatches a key up event for the specified key. It is typically
        used in conjunction with `key_down()` to simulate a full key press and release.
    ...
    async def _click_option_tag(self):
        ...
        This method is used internally to handle clicks on `<option>` HTML elements,
        which often require specific JavaScript interaction rather than a standard mouse click.
        """
        await self._execute_command(
    ...
    async def _is_element_visible(self):
        ...
        This method executes a JavaScript script to determine if the web element is
        currently visible in the browser's viewport. It considers various factors
        like display, visibility, opacity, and element dimensions.
    ...
    async def _is_element_on_top(self):
        ...
        This method determines if the web element is the topmost element at its center
        coordinates, meaning it is not obscured by other elements or overlays. This is
        useful for ensuring that interactions like clicks will target the intended element.

        Returns:
            bool: True if the element is on top, False otherwise.
        """
    ...
    def _def_attributes(self, attributes_list: list[str]):
        ...
        This internal method takes a flat list of attribute names and values (e.g.,
        `["id", "myId", "class", "myClass"]`) and converts it into a dictionary.
        It also renames the key 'class' to 'class_name' for consistency.

        Args:
            attributes_list (list[str]): A list of attribute names and their corresponding values.
        """
    ...
    def _is_option_tag(self):
        ...
        This internal method checks if the `tag_name` attribute of the web element
        is 'option', indicating that it is an HTML `<option>` element.
    ...
    def _calculate_center(bounds: list) -> tuple:
        ...
        This static method calculates the center coordinates (x, y) of a bounding box
        represented by a list of coordinates. It is used internally to determine the
        center of an element for mouse interactions.

        Args:
            bounds (list): A list of coordinates representing the bounding box.
    ...
    def __repr__(self):
        ...
        This method provides a string representation of the `WebElement` object,
        including its attributes and object ID, which is useful for debugging purposes.

        Returns:
            str: A string representation of the `WebElement` object.
        """
