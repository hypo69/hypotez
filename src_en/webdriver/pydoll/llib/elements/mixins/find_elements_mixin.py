## \file /src/webdriver/pydoll/llib/elements/mixins/find_elements_mixin.py
# -*- coding: utf-8 -*-
#! .pyenv/bin/python3
"""
.. module:: src.webdriver.pydoll.llib.elements.mixins.find_elements_mixin
    :platform: Windows, Unix
    :synopsis: Mixin for finding elements in a web page.

This module provides a mixin for finding elements on a web page using various strategies.
=========================================================================================

This module contains the `FindElementsMixin` class, which provides methods for finding and waiting for elements on a web page. It supports finding elements by ID, class name, name, tag name, text, and CSS selector, as well as by XPath. It also provides methods for waiting for elements to appear on the page.
...
```python
    ...
    class MyPage(FindElementsMixin):
        def __init__(self, connection_handler):
            self._connection_handler = connection_handler

        async def find_my_element(self):
            return await self.find(id='my-element')
```

:author: hypo69
:license: Proprietary. All rights reserved.
:version: 1.0.0
:location: src/webdriver/pydoll/llib/elements/mixins/find_elements_mixin.py
"""
...
from src.webdriver.pydoll.llib.constants import By, Scripts
from src.webdriver.pydoll.llib.exceptions import ElementNotFound, WaitElementTimeout
from src.webdriver.pydoll.llib.protocol.base import Command
from src.webdriver.pydoll.llib.protocol.dom.responses import DescribeNodeResponse
from src.webdriver.pydoll.llib.protocol.dom.types import Node
from src.webdriver.pydoll.llib.protocol.runtime.responses import (
...
def create_web_element(*args, **kwargs):
    ...
    """
    from src.webdriver.pydoll.llib.elements.web_element import WebElement  # noqa: PLC0415
...
class FindElementsMixin:
    ...
    async def query(
        ...
        return await self.find_or_wait_element(
            by=by, selector=expression, timeout=timeout, find_all=find_all, raise_exc=raise_exc
        )
    ...
    async def find_or_wait_element(
        ...
        if not timeout:
            return await find_method(by, selector, raise_exc=raise_exc)

        start_time = asyncio.get_event_loop().time()
        while True:
            element = await find_method(by, selector, raise_exc=raise_exc)
            if element:
                return element
    ...
    async def _find_element(
        self, by: By, selector: str, raise_exc: bool = True
    ) -> Optional['WebElement']:
        """
        ...
        if hasattr(self, '_object_id'):
            command = self._get_find_element_command(by, selector, self._object_id)
        else:
            command = self._get_find_element_command(by, selector)
    ...
    def _get_find_element_command(self, by: By, selector: str, object_id: str = ''):
        ...
        if object_id and not by == By.XPATH:
            ...
            command = self._get_find_element_by_xpath_command(selector, object_id)
        elif by == By.NAME:
            command = self._get_find_element_by_xpath_command(
                f'//*[@name="{escaped_selector}"]', object_id
            )
        else:
    ...
    def _get_find_elements_command(self, by: By, selector: str, object_id: str = ''):
        """
        ...
        match by:
            case By.CLASS_NAME:
                selector = f'.{_escaped_selector}'
            case By.ID:
                selector = f'#{_escaped_selector}'
            case _:
                selector = _escaped_selector
        ...
        if object_id and not by == By.XPATH:
            ...
            command = self._get_find_elements_by_xpath_command(selector, object_id)
        else:
            command = RuntimeCommands.evaluate(
