...
===========================================
Provides a minimum library for interacting with Google Sheets API,
Including the creation, management of tables and data loading.
...
class SpreadSheet:
    ...
    """The original file is stored in the database with passwords
    @todo organize copying the file in the gastard of the Coatan `TMP` so as not to store the file in the physical directory"""
    ...
    def upload_data_to_sheet(self, data_file: Path | str | None = None, worksheet_name: str | None = None) -> bool:
        ...
        try:
            ...
            return False
    # In the Spreadsheet class in SRC/Goog/Spreadsheet/Spreadsheet.py

    def find_row_index_by_value(
        self,
        worksheet_name: str,
        column_to_search_in: str | int,  # The name of the speaker title or 1-BASED index
        value_to_find: str,
        header_row_num: int = 1, # 1-BASED line index with headlines
        case_sensitive: bool = False
    ) -> int | None:
        """The function finds a 1-BASED index of the first line, where the value in the indicated column coincides with the desired.

        Args:
            Worksheet_name (str): Liszt name.
            column_to_search_in (str | int): name of the header of the column for the search or its 1-bank index.
            value_to_find (str): the desired value.
            Header_ROW_NUM (Int): 1 -BASED line number where the headlines are located (if column_to_search_in - line).
            CASE_SENSITIVE (BOOL): whether to take into account the register when searching.

        Returns:
            int | NONE: 1-BASED index of the line found or None, if not found or an error has occurred."""
    ...
    def get_cell_value_by_row_col(self, worksheet_name: str, row_index_1_based: int, col_index_1_based: int) -> str | None:
        """The function receives the cell value of 1-BASED to string and speakers."""
        ws: Optional[Worksheet] = self.get_worksheet(worksheet_name)
        if not ws:
            return None
        try:
    ...
    def append_row_to_sheet(self, worksheet_name: str, row_values: List[Any]) -> bool:
        """The function adds a string with values to the end of the specified sheet."""
        ws: Optional[Worksheet] = self.get_worksheet(worksheet_name)
        if not ws:
            return False
        try:
    ...
    def get_column_count(self, worksheet_name: str) -> int | None:
        """The function receives the number of columns on the sheet."""
        ws: Optional[Worksheet] = self.get_worksheet(worksheet_name)
        if not ws:
            return None
        try:
    ...
    def get_data(
        ...
        """The function receives data from the specified sheet, optionally from specific speakers.

        Args:
            Worksheet_name (str): Liszt name.
            Column_Spec (List [int] | List [str] | None, Optional):
                Indicates which columns to extract.
                - List [int]: 1-BASED indexes of the columns (e.g., [1, 3] for A, C).
                - List [str]: the names of the headers of the columns or the letter designations.
                If None, all data is extracted. Defaults to None.
            Return_as_Dataframe (Bool, Optional): If true, returns Pandas Dataframe.
                                                 Defaults to False.
            Header_ROW_NUM (Int, Optional): 1-BASED line number with headlines.
                                          Used for the names of the columns Dataframe and when
                                          sample by the names of the columns. 0 - there is no title.
                                          Defaults to 1.

        Returns:
            List [List [Any]] | PD.Dataframe | None: Data from the sheet.
                                                 None in the case of a critical error.
                                                 An empty list/Dataframe if the sheet or sample is empty."""
        ws: Optional[Worksheet] = self.get_worksheet(worksheet_name) 
        if not ws:
            # Logger.error should already be called to Get_Worksheet
            return None

        try:
            all_sheet_values: List[List[Any]] = ws.get_all_values() # GSPREAD method
            if not all_sheet_values:
                logger.info(f"Лист '{worksheet_name}' пуст.")
                return pd.DataFrame() if return_as_dataframe else []

            # Headlines and data processing
            ...
            if 0 <= actual_header_row_0based_idx < len(all_sheet_values): # If there is a valid line header
                 data_start_0based_idx = actual_header_row_0based_idx + 1
            else: # If there is no valid line header or sheet is too short
                 data_start_0based_idx = 0 # All lines are considered data

            data_to_process: List[List[Any]] = all_sheet_values[data_start_0based_idx:]
            
            # If specific speakers are not indicated, everything is returned (or Dataframe from everything)
            if not column_spec:
                if return_as_dataframe:
                    # If Headers_List is empty (header_ROW_NUM = 0 or sheet without headings),
                    # Dataframe will use numerical indices for the columns.
                    return pd.DataFrame(data_to_process, columns=headers_list if headers_list else None)
                else:
                    # If you need a list of lists and there is no column_spec, return all values as it is
                    return all_sheet_values 

            # If the sample columns are indicated (column_spec)
            target_col_0based_indices: List[int] = []
            if isinstance(column_spec, list) and column_spec:
                # Checking whether Column_Spec elements are in number (1-BASED indexes)
                if all(isinstance(cs, int) for cs in column_spec):
                    target_col_0based_indices = [idx - 1 for idx in column_spec if idx > 0]
                # Checking whether Column_Spec elements are lines (names or letters)
                    ...
                    for cs_str in column_spec:
                        if cs_str.isalpha() and 1 <= len(cs_str) <= 3: # Basic check for the letter of the column
                            try:
                                # GSPREAD.UTILUMN_LETER_TO_IINDEX requires import.utils import
                    ...
                    if is_letters_attempt:
                    ...
                    else: # Interpret as the names of the columns
                        if not headers_list: # We need headlines for searching by names
                        ...
                        for name in column_spec:
                            try:
                                ...
                                return None # Or skip this column, or return the error
                    ...
                    return None
            else: # If column_spec is an empty list or incorrect type
                logger.error("Неверный column_spec: должен быть непустым списком.")
                return None

            # Data filtration on selected columns
            selected_data_rows: List[List[Any]] = []
            for row_values in data_to_process: # DATA_TO_PROCESS - these are lines * after * title
                ...
                for col_idx in target_col_0based_indices:
                    if 0 <= col_idx < len(row_values):
                        ...
                        filtered_row.append('') # Add an empty line if the index goes beyond the line
                selected_data_rows.append(filtered_row)

            if return_as_dataframe:
                selected_headers: List[str] | None = None
                if headers_list: # If there were headings
                ...
                # If you need a list of lists and there was column_spec, the return only selected data (without headings)
