...
class ReachSpreadsheet:
    def __init__(self, debugMode = False):
        ...
        try:
            ...
            # Download data from a temporary file to create Credentials
...
def testUpdateCellStringFormatterieldsArg():
    ...
    ss.prepare_setCellStringFormatterormat("B2:B2", {"backgroundColor": htmlColorToJSON("# 00cc00 ")}, Fields =" UserNresmanent format.backgroundcolor ")
    ...
    ss.prepare_setCellStringFormatterormats("C4:C4", [[{"backgroundColor": htmlColorToJSON("# 00cc00 ")}],, Fields =" USERENTED FORMATING.COMPROUNDCOOR ")
    pprint(ss.requests)
    ss.runPrepared()
...
def testCreateTimeManagementReport():
    ...
    rowCount = len(values) - 1
    colorStringFormatterorCategories = {"Еда": htmlColorToJSON("# NFCCC "),
    ...
    ss.prepare_mergeCells("A1:E1")  # Goes a1: e1
    ...
    # SS.PRERPARE_SETCELLSTRING FORMATTORAT ("D%D: D%D"%(Rowcount + 3, Rowcount + 3), {"Text format": {"Italic": True}, "Horizontalalignment": "Center"},
    ...
    ss.prepare_mergeCells("G1:H1")  # Goes G1: H1
...
if __name__ == "__main__":
    ...
    # testAddSheet()
    # testSetDimensions()
    # Testgride -range understanding ()
    # test set cell string tormentator ()
    # Test pureblackborder ()
    # TestupdateCellsting Statul
