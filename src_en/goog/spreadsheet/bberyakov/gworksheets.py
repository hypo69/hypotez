...
class GWorksheet (Worksheet):
    ...
    def get (self, sh, ws_title: str = 'new', rows: int = 100, cols: int = 100, direction: str = 'rtl', wipe_if_exist: bool = True) :
        ...
        Parameters : 
             ...
             wipe_if_exist : bool = True : [description]"""
            """I create a new table in the book if WS_Title == 'New', \ N 
            Otherwise I open it by ws_title \ n
            `ws_title` (str) - table name (worksheet) in the book (Spreadsheet) \ n
            `rows` (int) - stake - lines \ n
            `colors` (int) - stake -in columns \ n
            `wipe_if_exist` (bool) - clean from old data"""
