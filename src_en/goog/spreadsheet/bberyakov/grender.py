...
class GSRender():
    ...
    def render_header (self, ws: Worksheet, world_title: str, range: str = 'A1:Z1', merge_type: str('MERGE_ALL') | str('MERGE_COLUMNS') | str('MERGE_ROWS') = 'MERGE_ALL' ) -> None:
        """I draw table title in the first line

        Parameters: 
             SELF: [Description]
             WS: Worksheet: table in the book
             world_title: str: header google table
             Range: 'A1: Z1': The Range of the Cells
             Merge_type: str ('Merge_all') | str ('merge_columns') | Str ('Merge_rows') = 'Merge_all': 'Merge_all' | 'Merge_columns' | 'Merge_rows'
        Returns: 
             None: [Description]"""
        bg_color = hex_to_rgb ('# Fraaa ')
    ...
    def merge_range (self, ws: Worksheet, range: str, merge_type: str('MERGE_ALL') | str('MERGE_COLUMNS') | str('MERGE_ROWS') =  'MERGE_ALL') -> None:
        ...
        Returns : 
             None : [description]"""
        """Section of the columns / lines \ n
        `ws` (gspread.worksheet) - table (worksheet) \ n
        `Merge_type`` str`: \ n \ t 'merge_all' | 'Merge_columns' | 'Merge_rows'"""
...
# None
    ...
    def get_first_empty_row (self, ws: Worksheet, by_col: int = None) -> int:
        ...
        Returns : 
             int : [description]"""
        """Returns the number of the last filled cell in the column if there are values in the table, otherwise 1 \ n
        `Worksheet` (GSPREAD.WORKSHEET) - Table (Worksheet) \ N
        `by_col` (int) - by which column to count (if by_col == none according to the latter completed in the table)"""
