## \file /src/suppliers/suppliers_list/aliexpress_com/campaign/prepare_campaigns.py
# -*- coding: utf-8 -*-
#! .pyenv/bin/python3
"""
...
def process_campaign(
    ...
    # Convert list of dictionaries to list of pairs (language, currency)
    _l = [(lang, curr) for locale in locales for lang, curr in locale.items()]
    #pprint(_l)

    # If language and currency are specified, filter the list by them
    if language and currency:
        _l = [(language, currency)]

    # Process each pair (language, currency)
    ...
    return True  # Assume campaign is always processed successfully
