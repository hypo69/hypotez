## \file /src/suppliers/suppliers_list/aliexpress_com/campaign/ali_promo_campaign.py
# -*- coding: utf-8 -*-
#! .pyenv/bin/python3

"""
.. module:: src.suppliers.suppliers_list.aliexpress_com.campaign.ali_promo_campaign
...
### Purpose:
This module is designed to manage advertising campaigns on the AliExpress platform, including processing category and product data, creating and editing JSON files with campaign information, and using AI to generate campaign data.

### Description:
The `AliPromoCampaign` class allows loading and processing advertising campaign data, managing categories and products, and using AI to generate descriptions and other data. The module supports various languages and currencies, providing flexibility in campaign setup.

### Examples:
Example of initializing an advertising campaign:

    >>> campaign = AliPromoCampaign("new_campaign", "EN", "USD")
    >>> print(campaign.campaign_name)

Example of processing an entire campaign:
...
Example of populating category data using AI:
...
import header
from src import gs
from src.suppliers.suppliers_list.aliexpress_com import campaign
...
class AliPromoCampaign:
    ...
    def __init__(
        ...
        Args:
            campaign_file (Optional[str | Path]): Path to the campaign file or link to download the campaign.
            campaign_name (Optional[str]): Name of the campaign.
            language (Optional[str | dict]): Language used in the campaign.
            currency (Optional[str]): Currency used in the campaign.

        Returns:
            SimpleNamespace: Object representing the campaign.
        ...
        self.campaign = j_loads_ns(
            campaign_file_path,
            exc_info=False
        )  # <- file may not exist if I am creating a new advertising campaign - the file will be created by AI
        if not self.campaign:
            logger.warning(
                f"Campaign file not found at {campaign_file_path=}\nStart as new \n (Start build JSON file, categories, products etc.)"
            )
            """ If there is no JSON file in the root of the advertising campaign -> the process of creating a new advertising campaign starts
            categories will be created from directory names in the `category` directory,
            affiliated products will be collected into <product_id>.JSON files
            ai parameters will be generated
            """
    ...
    def _models_payload(self):
        ...
        assistant_id = "asst_dr5AgQnhhhnef5OSMzQ9zdk9" # <-  task asst_dr5AgQnhhhnef5OSMzQ9zdk9 creating categories and descriptions based on a list of product names
        #self.openai = OpenAIModel(system_instruction = system_instruction, assistant_id = assistant_id)

    def process_campaign(self):
        """The function iterates through the categories of the advertising campaign and processes the category products through the affiliate link generator.

        Example:
            >>> campaign.process_campaign()
        """
        ... 
        categories_names_list = get_directory_names(self.base_path / 'category') # read category folder names
    ...
    def process_new_campaign(
        ...
        """Creating a new advertising campaign.
        Conditions for creating a campaign:
        - campaign directory with pythonic name
        - nested `campaign` directory, with pythonic category names
        - sources.txt file and/or `sources` directory with `<product_id>.html` files

        Args:
            campaign_name (Optional[str]): The name of the advertising campaign.
            language (Optional[str]): Language for the campaign (optional).
            currency (Optional[str]): Currency for the campaign (optional).

        Returns:
            List[Tuple[str, Any]]: List of tuples with category names and their processed results.
        ...
        for language, currency in _l:
            ...
            self.campaign_ai = copy.copy(
                self.campaign
            )  # <- create AI campaign in parallel
            ...
            for category_name in self.campaign.category.__dict__:
                ...
                )  # <- to the newly created category file
    ...
    def process_llm_category(self, category_name: Optional[str] = None):
        ...
        def _process_category(category_name: str):
            ...
            try:
                res_ns: SimpleNamespace = j_loads_ns(response)  # <- convert machine response to SimpleNamespace object
    ...
    def dump_category_products_files(
        ...
        """Saves product data to JSON files.
    ...
    def set_categories_from_directories(self):
        """Sets advertising campaign categories from directory names in `category`.

        Converts each element of the category list into a `SimpleNamespace` object with attributes
        `category_name`, `title`, and `description`.
    ...
    def generate_html_for_campaign(self, campaign_name: str):
        ...
        # Generate advertising campaign page
