...
Этот файл документирует структуру и назначение локаторов веб-элементов, Используетсяых в проекте `hypotez`. Локаторы представляют собой JSON-объекты, которые определяют, как веб-драйвер должен найти конкретный элемент на странице. 
