...
Этот модуль предоставляет кастомную реализацию `PlaywrightCrawler` с использованием библиотеки Crawlee. 
...
- [Конфигурация](#конфигурация)
    ...
    - [Описание полей конфигурации](#описание-полей-конфигурации)
        - [1. `max_requests`](#1-max_requests)
        - [2. `headless`](#2-headless)
        - [3. `browser_type`](#3-browser_type)
        - [4. `options`](#4-options)
        - [5. `user_agent`](#5-user_agent)
        - [6. `proxy`](#6-proxy)
        - [7. `viewport`](#7-viewport)
        - [8. `timeout`](#8-timeout)
        - [9. `ignore_https_errors`](#9-ignore_https_errors)
