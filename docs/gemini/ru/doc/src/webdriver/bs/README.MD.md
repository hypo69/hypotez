...
Этот модуль предоставляет пользовательскую реализацию для разбора HTML-контента с использованием BeautifulSoup и XPath. Он позволяет получать HTML-контент из файлов или URL-адресов, анализировать его и извлекать элементы с использованием локаторов XPath.

## Подробнее

Модуль `src.webdriver.bs` предназначен для разбора HTML-контента с использованием библиотек `BeautifulSoup` и `lxml` для XPath. Он предоставляет возможности для получения HTML-контента как из локальных файлов, так и из веб-URL, а также позволяет извлекать элементы на основе пользовательских XPath-локаторов. Централизованная конфигурация осуществляется через файл `bs.json`, что упрощает настройку и управление параметрами разбора.
...
- **Пользовательские локаторы**: Позволяет определять пользовательские локаторы XPath для извлечения элементов.
...
Конфигурация для парсера `BS` хранится в файле `bs.json`. Ниже приведена структура файла конфигурации и его описание:
...
- **selector**: Селектор XPath для извлечения элементов.
