...
Модуль `src.webdriver.playwright.py` предоставляет кастомную реализацию `PlaywrightCrawler` с использованием библиотеки Playwright. Он позволяет настраивать параметры запуска браузера, такие как user-agent, прокси, размер окна и другие настройки, определённые в файле `playwrid.json`.
...
class Playwrid:
    """
    Класс для запуска браузера с помощью Playwright.

    Attributes:
        options (list): Список аргументов командной строки для запуска браузера.
        user_agent (str): Строка user-agent, которая будет использоваться для запросов.
        headless (bool): Флаг, указывающий, следует ли запускать браузер в безголовом режиме.
        browser_type (str): Тип браузера, который будет использоваться.
        timeout (int): Максимальное время ожидания для выполнения операций.
        ignore_https_errors (bool): Флаг, указывающий, следует ли игнорировать ошибки HTTPS.
        proxy (dict): Настройки прокси-сервера.
        viewport (dict): Размеры окна браузера.
        browser (Browser): Экземпляр браузера Playwright.

    Methods:
        start(url: str) -> None: Запускает браузер и открывает указанный URL.
        close() -> None: Закрывает браузер.
    """
```
...
**Назначение**: Запускает браузер и открывает указанный URL.

**Параметры**:

- `url` (str): URL, который будет открыт.

##### `close() -> None`

**Назначение**: Закрывает браузер.
