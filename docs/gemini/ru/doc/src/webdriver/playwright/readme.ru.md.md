...
Этот модуль предназначен для автоматизации браузера с использованием Playwright. Он обеспечивает централизованную конфигурацию, поддержку пользовательских опций, улучшенное логирование и обработку ошибок, а также гибкие настройки браузера, включая прокси, размер окна и user-agent.
