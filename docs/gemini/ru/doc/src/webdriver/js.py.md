...
* Используй `driver.execute_locator(l: dict)` для выполнения команд Selenium, например, для поиска элементов.
* Используй `logger` из модуля `src.logger` для регистрации ошибок и информации.
