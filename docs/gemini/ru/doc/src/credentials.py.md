...
Пути к бинарным директориям добавляются в `sys.path` и переменные окружения (`WEASYPRINT_DLL_DIRECTORIES`), что необходимо для корректной работы некоторых внешних инструментов, Используетсяых проектом (например, `wkhtmltopdf`, `ffmpeg`, `graphviz`, `gtk`).
