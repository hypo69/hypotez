...
Модуль `facebook_fields` обеспечивает доступ к полям, Используетсяым для Facebook-объявлений и событий. 
...
**Описание**: Класс `FacebookFields`  предоставляет доступ к полям, Используетсяым для Facebook-объявлений и событий. 
**Наследует**: 
**Атрибуты**:
