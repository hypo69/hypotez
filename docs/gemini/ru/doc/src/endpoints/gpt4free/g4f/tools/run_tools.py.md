...
Модуль `run_tools` содержит функциональность для обработки различных типов инструментов, Используетсяых в модели `GPT4Free`. 
...
**Описание**: Класс `ToolHandler` отвечает за обработку различных типов инструментов, Используетсяых моделью `GPT4Free`.
