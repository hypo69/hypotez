...
- `model (str)`: Имя модели GPT, Используетсяой для генерации текста.
- `messages (Messages)`: Список сообщений, Используетсяых в качестве контекста для генерации текста.
