...
Модуль `retry_provider.py` предоставляет механизм повторных попыток (retries) для получения результатов от разных провайдеров, Используетсяых для генерации текста. 
...
- `providers` (List[Type[BaseProvider]]): Список провайдеров, Используетсяых для генерации текста.
