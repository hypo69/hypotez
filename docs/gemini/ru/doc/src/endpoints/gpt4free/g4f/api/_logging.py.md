...
- Комментарии к неИспользуетсяому коду: Этот модуль содержит закомментированный код, который был предназначен для использования `loguru` для ведения логов, но в конечном итоге был заменен стандартным модулем `logging`. 
...
- Комментарии к неИспользуетсяому коду предоставляют контекст для понимания истории развития этого модуля.
