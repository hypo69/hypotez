...
- `client` (`AsyncClient`): Объект клиента GPT-4Free, Используетсяый для взаимодействия с API.
- `model` (`str`): Название модели GPT-4Free, Используетсяой для генерации аудио.
