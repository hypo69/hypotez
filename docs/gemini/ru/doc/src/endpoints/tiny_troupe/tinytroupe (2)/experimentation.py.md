...
- `random_seed` (int): Случайное семя, Используетсяое для генерации случайных чисел.
...
- Используй классы `ABRandomizer` и `Intervention` для проведения экспериментов в среде с агентами.
