...
- `get_product_images`: Получает изображения продукта асинхронно.
...
**Назначение**: Получает изображения продукта асинхронно.
...
**Как работает функция**:
- Вызывает метод `_exec` для получения списка изображений продукта.
