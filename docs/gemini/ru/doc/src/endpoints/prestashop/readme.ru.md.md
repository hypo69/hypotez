...
В этом документе представлена информация о том, как управлять сайтами, работающими на платформе PrestaShop. Рассматривается структура сайтов, способы взаимодействия с ними через API, а также безопасное хранение ключей API, необходимых для доступа к сайтам.
