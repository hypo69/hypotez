...
Модуль `src/check_release.py` предоставляет функцию для проверки последней версии релиза репозитория GitHub.  Он используется для получения информации о текущей версии проекта, чтобы убедиться, что Используетсяая версия является актуальной.
