...
- `driver (Driver)`: Экземпляр веб-драйвера, Используетсяый для взаимодействия со страницей. 
- `url (str)`: Полный URL-адрес страницы поставщика, с которого нужно получить информацию.
- `lang_index (int)`: Индекс языка, Используетсяый для локализации информации о товаре.
