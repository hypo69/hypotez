...
Данный модуль используется для управления типом товара, сортировкой и типом ссылки в запросах к API AliExpress. Он предоставляет набор классов, каждый из которых определяет константы, представляющие различные варианты параметров запроса.

## Классы

### `ProductType`

**Описание**: Класс `ProductType` определяет типы товаров для запросов к API AliExpress.

**Атрибуты**:

- `ALL` (str): Все типы товаров.
- `PLAZA` (str): товары с площадки PLAZA.
- `TMALL` (str): товары с площадки TMALL.
