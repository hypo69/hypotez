...
Функция использует функцию `extract_prod_ids` из модуля `src.suppliers.suppliers_list.aliexpress_com.utils.extract_product_id` для извлечения ID товара. 

**Примеры**:

```python
>>> get_product_id("https://www.aliexpress_com.com/item/10000000000000.html")
'10000000000000'
...
src.suppliers.suppliers_list.aliexpress_com.errors.ProductIdNotFoundException: Product id not found: No product ID here.
```