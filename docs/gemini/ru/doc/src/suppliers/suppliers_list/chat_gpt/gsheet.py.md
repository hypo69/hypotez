...
Он использует класс `SpreadSheet` из `src.goog.spreadsheet.spreadsheet` для взаимодействия с Google Sheets и позволяет  сохранять, обновлять и извлекать данные о категориях и продуктах в кампании.

## Подробнее

`GptGs` предоставляет методы для:

- Очистки таблицы: удаления листов продуктов и очистки данных в категориях и других листах.
...
- Записи данных продукта: записи данных о продукте в новый лист Google Sheets.
- Чтения данных продукта: чтения данных о продукте из листа "products".
- Записи данных продуктов: записи данных о продуктах в лист "products".
- Удаления листов продуктов: удаления всех листов, кроме "categories", "product" и "product_template" из таблицы Google Sheets.
...
- `set_product_worksheet`: Записывает данные о продукте в новый лист Google Sheets.
- `get_product_worksheet`: Читает данные о продукте из листа "products".
- `set_products_worksheet`: Записывает данные о продуктах в лист "products".
...
**Как работает функция**:
- Удаляет листы продуктов с помощью метода `delete_products_worksheets`.
...
**Назначение**: Записывает данные о продукте в новый лист Google Sheets.
...
- Извлекает данные из объекта `SimpleNamespace` в переменные, представляющие данные о продукте.
- Записывает данные о продукте во вторую строку листа.
...
**Назначение**: Читает данные о продукте из листа "products".
...
- Читает все значения из листа с помощью метода `get_all_values`.
- Создает объект `SimpleNamespace` с данными о продукте, извлеченными из листа.
...
**Назначение**: Записывает данные о продуктах в лист Google Sheets.

**Параметры**:

- `category_name` (`str`): Имя категории, для которой нужно записать данные о продуктах.
