# Модуль для нормализации строк и числовых данных (normalizer.py)

## Обзор

Этот модуль предоставляет функции для нормализации строк, булевых значений, целых чисел и чисел с плавающей точкой. Он также содержит вспомогательные методы для обработки текста, включая удаление HTML-тегов и специальных символов.

## Подробней

Модуль `src.utils.string.normalizer` предоставляет набор утилит для очистки и преобразования различных типов данных. Он включает функции для нормализации строк (удаление HTML-тегов, пробельных символов и специальных символов), преобразования данных в булевы значения, целые числа и числа с плавающей точкой, а также для нормализации дат в формат SQL.

## Функции

### `normalize_boolean`

**Назначение**: Нормализует данные в булево значение.
...
-   `input_data` (Any): Данные, которые можно представить в виде булева значения (например, bool, string, integer).
...
1.  Проверяет, является ли входное значение булевым. Если да, возвращает его без изменений.
2.  Преобразует входное значение в строку, удаляет пробельные символы и приводит к нижнему регистру.
3.  Проверяет, соответствует ли строка одному из значений, представляющих `True` (`'true'`, `'1'`, `'yes'`, `'y'`, `'on'`, `True`, `1`).
4.  Если строка не соответствует ни одному из значений `True`, проверяет, соответствует ли она одному из значений, представляющих `False` (`'false'`, `'0'`, `'no'`, `'n'`, `'off'`, `False`, `0`).
5.  Если строка не соответствует ни одному из известных значений, логирует предупреждение и возвращает исходное значение.
...
1.  Проверяет тип входных данных. Если это не строка и не список, вызывает `TypeError`.
2.  Если входные данные - список, объединяет строки списка в одну строку, разделяя их пробелами.
3.  Удаляет HTML-теги, переносы строк и специальные символы из строки, используя функции `remove_html_tags`, `remove_line_breaks` и `remove_special_characters`.
4.  Удаляет лишние пробельные символы и возвращает строку в формате UTF-8.

### `normalize_int`

**Назначение**: Нормализует данные в целое число.
...
1.  Преобразует входные данные в число с плавающей точкой, а затем в целое число.
2.  Обрабатывает возможные исключения, такие как `ValueError`, `TypeError` и `InvalidOperation`, логируя ошибки и возвращая исходное значение в случае ошибки.
...
3.  Пытается преобразовать входное значение в число с плавающей точкой, используя `float(value)`.
...
2.  Пытается преобразовать входные данные в формат даты SQL, используя различные форматы даты.
3.  Если преобразование удалось, возвращает дату в формате YYYY-MM-DD.
4.  В случае ошибки логирует ошибку и возвращает исходное значение.

### `simplify_string`

**Назначение**: Упрощает входную строку, оставляя только буквы и цифры, заменяя пробелы на подчеркивания.
...
**Назначение**: Нормализует SKU (Stock Keeping Unit), удаляя определенные ключевые слова и специальные символы.
