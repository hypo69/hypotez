...
2. **Получение информации о языках:**
   - Используй метод `get_languages_schema()` для получения актуального списка языков, доступных в магазине PrestaShop.
...
4. **Добавление, удаление и обновление языков:**
   - Используй методы `add_language_PrestaShop(lang_name, lang_iso)`, `delete_language_PrestaShop(lang_id)` и `update_language_PrestaShop(lang_id, new_lang_name)` для управления языками в PrestaShop.
