...
2. **Получите доступ к ключам API**: Ключи API хранятся в файле `credentials.kdbx`. Используй менеджер паролей, поддерживающий формат `.kdbx`, такой как KeePass или KeePassXC, для доступа к ключам.
3. **Подключитесь к API**: Используй пример запроса API для подключения к API вашего сайта PrestaShop.
