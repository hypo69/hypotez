...
1. **Создание экземпляра:** 
    ...
    ```
2. **Заполнение полей:**
    - Используй свойства (геттеры и сеттеры) для установки значений полей товара.
    ...
    ```
3. **Добавление связей:**
    - Используй специальные методы для добавления связей товара с категориями, изображениями, характеристиками, тегами и т.д.
    ```python
...
4. **Получение словаря для API:**
    - Используй метод `to_dict()` для получения словаря Python, содержащего все данные товара в формате, совместимом с API PrestaShop.
    ```python
    api_dict = product.to_dict()
    ```
5. **Использование словаря в API:**
    - Используй полученный словарь `api_dict` для создания нового товара или редактирования существующего товара в PrestaShop через API. 
