...
Описание
-------------------------
Этот код реализует телеграм-бота для обработки запросов, связанных с созданием прайс-листов для компании Казаринова. Бот обрабатывает команды, URL-адреса OneTab, текстовые и голосовые сообщения, а также документы, предоставляя пользователям информацию и выполняя сценарии на основе полученных данных.

Шаги выполнения
-------------------------
1. **Инициализация бота**: Создание экземпляра `telebot.TeleBot` с использованием токена, полученного из переменных окружения или учетных данных Google Sheets.
2. **Обработка команд**: Регистрация обработчиков для команд `/start`, `/help`, `/info`, `/time` и `/photo`. Каждый обработчик логирует информацию о пользователе, отправившем команду, и выполняет соответствующие действия, такие как отправка приветственного сообщения, справки, информации о боте, текущего времени или случайной фотографии.
3. **Обработка голосовых сообщений**: При получении голосового сообщения вызывается функция `handler.handle_voice`, которая извлекает информацию о файле, скачивает его, сохраняет во временный файл, транскрибирует текст (в данном случае, заглушка) и отправляет распознанный текст пользователю.
4. **Обработка документов**: При получении документа вызывается функция `handler.handle_document`, которая извлекает информацию о файле, скачивает его, сохраняет во временный файл и отправляет сообщение пользователю с путем к сохраненному файлу.
5. **Обработка текстовых сообщений**: Если сообщение не начинается с `/`, оно передается в функцию `handler.handle_message`, которая обрабатывает URL-адреса OneTab, команды `--next` и обычные текстовые сообщения, отправляя ответы пользователю.
6. **Обработка неизвестных команд**: Если сообщение начинается с `/`, но не соответствует ни одной из известных команд, вызывается функция `handle_unknown_command`, которая отправляет пользователю сообщение о неизвестной команде.
7. **Запуск бота**: Функция `run_bot` запускает бота в бесконечном цикле с использованием `bot.infinity_polling()`, обеспечивая автоматическое восстановление при возникновении ошибок.
