...
2. **Обработка данных**:
    - Используй метод `process_graber` для обработки данных с сайтов поставщиков:
        - Передайте в метод список `urls` - URL-адреса страниц с товарами.
        - Опционально можно передать:
            - `price`: Цену товара.
            - `mexiron_name`: Название товара.
            - `scenarios`: Сценарий исполнения для обработки данных, который находится в директории `src.suppliers.suppliers_list.<supplier>.scenarios`.
    - Используй метод `process_scenarios` для обработки данных из JSON-файлов.
