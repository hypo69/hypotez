...
В этом примере мы Используется функцию `_create_completion` для отправки запроса с сообщением "Привет, как дела?" к модели `gpt-3.5-turbo-16k`. Параметр `stream=True` активирует потоковую передачу ответов.  Каждый полученный фрагмент ответа выводится на консоль.