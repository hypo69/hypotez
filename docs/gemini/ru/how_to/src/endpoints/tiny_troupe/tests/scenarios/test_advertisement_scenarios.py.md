...
Описание
-------------------------
Этот блок кода реализует сценарий тестирования, имитирующий взаимодействие виртуальных агентов с рекламными объявлениями. Сценарий состоит из двух частей:

- **Оценка рекламы:** виртуальные агенты оценивают представленные рекламные объявления и выбирают наиболее привлекательное для себя.
- **Создание рекламы:** виртуальные агенты обсуждают и предлагают идеи для рекламы конкретного объекта, в данном случае - квартиры.

Шаги выполнения
-------------------------
1. **Подготовка:**  
    - Загружаются необходимые библиотеки, включая `tinytroupe` для работы с виртуальными агентами.
    - Создается контекст для запуска сценария.
    - Подготавливаются тексты рекламных объявлений.
2. **Оценка рекламы:**
    - Создается контекст ситуации: "Пользователь решил посетить Европу и планирует отпуск, ищет выгодные предложения и идеи".
    - Формулируется задача для агентов: "Оцените рекламу и выберите наиболее привлекательную. Обоснуйте свой выбор, опираясь на личный опыт и предпочтения".
    - Создаются два виртуальных агента с разным фоном (`create_oscar_the_architect` и `create_lisa_the_data_scientist`).
    - Агентам предоставляется контекст и задание.
    - Агенты анализируют рекламу и выбирают понравившееся объявление.
    - Извлекаются результаты, включая номер выбранного объявления, заголовок и обоснование выбора.
    - Проводится проверка результатов:
        - Убеждаемся, что результаты извлечены корректно.
        - Проверяем, что выбранное объявление соответствует одному из четырех вариантов.
        - Убеждаемся, что агенты дали обоснование своего выбора.
3. **Создание рекламы:**
    - Задается контекст: "Группа людей, знакомых с Пауло, обсуждают, как лучше рекламировать его квартиру для аренды".
    - Предоставляется описание квартиры и задание: "Обсудите, как лучше рекламировать квартиру, чтобы найти хорошего арендатора".
    - Запускается симуляция для двух агентов.
    - Извлекаются результаты обсуждения.
    - Проверяется, что результаты соответствуют задаче.
4. **Профилирование потребителей:**
    - Задается общий контекст: "Изучение рынка сбыта консервированного гаспачо, опрашиваются различные люди с разным опытом и взглядами".
    - Создается фабрика для генерации виртуальных потребителей.
    - Проводится симуляция интервьюирования 15 потребителей.
    - Каждый потребитель предоставляет информацию о себе и выражает мнение о покупке консервированного гаспачо.
    - Проверяется, что результаты сохраняются в файл.
