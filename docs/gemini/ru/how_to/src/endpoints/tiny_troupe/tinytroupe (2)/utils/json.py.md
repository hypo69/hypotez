...
Описание
-------------------------
Этот код предоставляет класс `JsonSerializableRegistry`, который позволяет объектам быть сериализованными и десериализованными в формат JSON. Также определены вспомогательные функции для работы со словарями и списками.

Шаги выполнения
-------------------------
1. **Определение класса `JsonSerializableRegistry`**:
   - Класс содержит методы для сериализации (`to_json`) и десериализации (`from_json`) объектов в JSON.
   - Метод `to_json` преобразует объект в JSON-представление, учитывая атрибуты, которые нужно включить или исключить.
   - Метод `from_json` создает экземпляр класса из JSON-представления.
   - Метод `__init_subclass__` регистрирует подклассы и расширяет атрибуты сериализации.
   - Методы `_programmatic_name_to_json_name` и `_json_name_to_programmatic_name` преобразуют имена атрибутов между стилями.

2. **Функция `post_init`**:
   - Декоратор, который обеспечивает вызов метода `_post_init` после инициализации класса.

3. **Функция `merge_dicts`**:
   - Выполняет слияние двух словарей, обрабатывая различные типы данных и конфликты.

4. **Функция `remove_duplicates`**:
