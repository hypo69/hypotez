...
2. **Настройка персональных данных**:
   - Используй метод `define` для установки различных атрибутов персонажа, таких как возраст, профессия, интересы и навыки. 
...
3. **Определение отношений с другими агентами**:
   - Используй метод `related_to` для установки отношений между агентом и другими агентами в симуляции. 
   - Отношения могут быть симметричными (взаимными) или односторонними.

4. **Обучение агента**:
   - Используй методы `read_documents_from_folder`, `read_document_from_file`, `read_documents_from_web` или `read_document_from_web` для загрузки документов в семантическую память агента. 
   - Это позволит агенту получать знания из внешних источников.

5. **Взаимодействие с окружающей средой и другими агентами**:
   - Используй методы `listen`, `see`, `socialize` и `think` для имитации восприятия агентом различных стимулов.
   - Используй метод `act` для выполнения действий в среде. Действия могут выполняться до тех пор, пока агент не завершит свою деятельность, или заданное количество раз.

6. **Управление памятью**:
   - Используй метод `store_in_memory` для сохранения информации о событиях и действиях в памяти агента.
   - Используй методы `retrieve_memories`, `retrieve_recent_memories` и `retrieve_relevant_memories` для извлечения информации из памяти агента.

7. **Управление когнитивным состоянием**:
   - Используй методы `move_to` и `change_context` для изменения местоположения и контекста агента.
   - Используй метод `_update_cognitive_state` для обновления целей, внимания и эмоций агента.

8. **Сохранение и загрузка состояния агента**:
   - Используй метод `save_specification` для сохранения конфигурации агента в JSON-файл.
   - Используй метод `load_specification` для загрузки конфигурации агента из JSON-файла.

9. **Взаимодействие с агентом в симуляции**:
   - Используй методы `listen_and_act`, `see_and_act` и `think_and_act` для объединения восприятия и действия в один шаг.

10. **Отображение информации об агенте**:
    - Используй метод `minibio` для получения краткой биографии агента.
    - Используй методы `pp_current_interactions` и `pretty_current_interactions` для просмотра текущих взаимодействий агента.
