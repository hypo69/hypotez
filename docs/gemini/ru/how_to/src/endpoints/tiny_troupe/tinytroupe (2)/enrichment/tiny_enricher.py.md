...
**Объяснение**: В этом примере мы создание экземпляр TinyEnricher, задаем требования к обогащению и контент, который нужно обогатить. Затем вызываем метод `enrich_content`, который отправляет запрос в OpenAI API, обрабатывает ответ и возвращает результат.