...
В этом примере мы создание объект `Driver` с использованием браузера Chrome, а затем вызываем функцию `switch_account`, передавая ей объект `driver`. Это позволит переключить аккаунт в Facebook, если кнопка "Переключить" присутствует на странице.