...
В данном примере мы создание список сообщений, включающий приветствие. Затем мы вызываем функцию `_create_completion` с моделью `gpt-3.5-turbo` и списком сообщений. Полученный генератор `completion` позволяет нам получать текст по частям в потоковом режиме, который мы выводим на консоль.