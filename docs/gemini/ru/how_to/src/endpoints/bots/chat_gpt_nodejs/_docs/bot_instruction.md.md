...
- `!train <data> <data_dir> <positive> <attachment>`: Обучает модель на предоставленных данных.  Используй `data` для файла, `data_dir` для каталога или `attachment` для вложения файла. 
