...
1. **Создание экземпляра класса:**
   - Используй метод `Mega.from_credentials(email, password)` для создания экземпляра, авторизованного с помощью email и пароля.
   - Используй метод `Mega.from_ephemeral()` для создания экземпляра, авторизованного с помощью ephemeral учетной записи.
2. **Выполнение API-запросов:**
   - Используй метод `api_req(data)` для отправки API-запроса к сервису Mega.
   - Метод `api_req` принимает в качестве параметра словарь `data`, который содержит информацию о запросе.
3. **Загрузка файла:**
   - Используй метод `uploadfile(filename, dst=None)` для загрузки файла на Mega.
...
4. **Скачивание файла:**
   - Используй метод `download_file(file_id, file_key, public=False, store_path=None)` для скачивания файла с Mega.
...
5. **Получение публичной ссылки:**
   - Используй метод `get_public_url(file_id, file_key)` для получения публичной ссылки на файл.
   - Метод принимает идентификатор файла `file_id` и ключ файла `file_key`.
