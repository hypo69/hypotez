...
2. **Используй методы класса BaseDAO** для взаимодействия с базой данных. 
...
6. **Используй методы `find_one_or_none_by_id`, `find_one_or_none`, `find_all`, `add`, `add_many`, `update`, `delete`, `count`, `paginate`, `find_by_ids`, `upsert`, `bulk_update`** для соответствующих операций.
...
- Используй `logger.info` для вывода информационных сообщений о выполняемых операциях.
- Используй `logger.error` для вывода ошибок, возникающих во время взаимодействия с базой данных.
