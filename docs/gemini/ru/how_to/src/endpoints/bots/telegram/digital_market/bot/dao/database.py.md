...
В примере кода мы создание модель `User`, которая наследуется от `Base`, получая  общие атрибуты  `id`, `created_at`, `updated_at`.  
Затем мы создание функцию `create_user`, которая создает нового пользователя в базе данных и возвращает его объект.  
В функции `main`  мы создание нового пользователя с заданными данными и выводим его в виде словаря с помощью `to_dict()`.
