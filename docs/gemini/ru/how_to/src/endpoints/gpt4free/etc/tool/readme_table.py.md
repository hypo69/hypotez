...
Описание
-------------------------
Этот код предназначен для автоматического создания документации в формате Markdown о доступных провайдерах и моделях в библиотеке `g4f` (gpt4free). Он тестирует доступность провайдеров, собирает информацию о поддерживаемых моделях, функциях (таких как streaming и message history) и генерирует таблицы Markdown для включения в документацию.

Шаги выполнения
-------------------------
1. **Импорт необходимых модулей**:
   - Импортируются различные модули из библиотеки `g4f`, такие как `models`, `ChatCompletion`, `ProviderType` и другие, а также стандартные библиотеки `re`, `urllib.parse` и `asyncio`.

2. **Определение асинхронной функции `test_async`**:
   - Функция `test_async` принимает объект провайдера (`ProviderType`) в качестве аргумента.
   - Проверяет, является ли провайдер рабочим (`provider.working`). Если нет, возвращает `False`.
   - Пытается выполнить асинхронный запрос к провайдеру с использованием `ChatCompletion.create_async`.
   - Возвращает `True`, если запрос выполнен успешно, и `False` в случае ошибки или исключения.

3. **Определение функции `test_async_list`**:
   - Функция `test_async_list` принимает список провайдеров (`list[ProviderType]`) в качестве аргумента.
   - Асинхронно тестирует каждый провайдер из списка с использованием `test_async` и собирает результаты в список `responses`.
   - Возвращает список результатов.

4. **Определение функции `print_providers`**:
   - Функция `print_providers` не принимает аргументов.
   - Фильтрует список провайдеров, оставляя только рабочие (`provider.working == True`).
   - Получает результаты тестирования провайдеров с помощью `test_async_list`.
   - Формирует список строк `lines` для Markdown-таблицы, включая информацию о каждом провайдере: имя, веб-сайт, статус, поддержку streaming, system message и message history.
   - Добавляет информацию о моделях, поддерживаемых провайдером, если провайдер является подклассом `ProviderModelMixin`.
   - Возвращает список строк `lines`.

5. **Определение функции `print_models`**:
   - Функция `print_models` не принимает аргументов.
   - Формирует список строк `lines` для Markdown-таблицы, включающей информацию о моделях: имя, базовый провайдер, провайдер и веб-сайт.
   - Использует информацию из `models.ModelUtils.convert` для получения данных о моделях.
   - Возвращает список строк `lines`.

6. **Определение функции `print_image_models`**:
   - Функция `print_image_models` не принимает аргументов.
   - Формирует список строк `lines` для Markdown-таблицы, включающей информацию о провайдерах, поддерживающих генерацию изображений: имя, image models, vision models и веб-сайт.
   - Возвращает список строк `lines`.

7. **Основной блок `if __name__ == "__main__":`**:
   - Открывает файл `docs/providers.md` для записи.
   - Записывает в файл результаты, возвращаемые функциями `print_providers` и `print_image_models`, разделенные разделителем `---`.
