...
В этом примере мы создание экземпляр класса `FastGpt` и передаем ему список сообщений для диалога, используя модель `gpt-3.5-turbo`. Затем мы вызываем метод `create_completion` с флагом `stream=True`, чтобы использовать потоковую передачу. В цикле `for` мы выводим каждый полученный токен, формируя ответ от модели.