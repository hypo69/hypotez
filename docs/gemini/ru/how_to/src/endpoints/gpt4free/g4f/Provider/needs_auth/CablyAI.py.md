...
3. **Создание асинхронного генератора**:  Используй метод `create_async_generator` для получения асинхронного генератора, который будет генерировать ответы от модели.
4. **Взаимодействие с моделью**:  Используй полученный генератор для отправки запросов к модели и получения ответов. 
