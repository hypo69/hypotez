...
2. **Вызов метода `create_async_generator`:** Используй метод `create_async_generator` для получения асинхронного генератора ответов. Метод принимает следующие аргументы:
...
- Класс `Goabror` использует асинхронный код. Используй его в асинхронных функциях или в цикле `asyncio.run`.
