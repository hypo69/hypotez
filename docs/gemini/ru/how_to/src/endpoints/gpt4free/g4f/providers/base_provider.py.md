### Как использовать класс `AsyncAuthedProvider`
=========================================================================================

Описание
-------------------------
`AsyncAuthedProvider` - это класс, который предоставляет асинхронную функциональность генератора для стриминга результатов с использованием аутентификации. Этот класс предназначен для работы с провайдерами, требующими аутентификацию через API ключ. Он автоматически управляет кэшированием результатов аутентификации для последующих запросов.

Шаги выполнения
-------------------------
1. **Аутентификация**: Класс сначала пытается получить данные аутентификации из кэш-файла. Если кэш-файл существует и содержит валидные данные, они используются для создания аутентифицированного запроса.
2. **Создание запроса**: Если кэш-файл не существует или содержит невалидные данные, вызывается метод `on_auth_async` для выполнения процесса аутентификации. Этот метод должен быть переопределен в подклассах для предоставления конкретной логики аутентификации.
3. **Кэширование результата**: После успешной аутентификации результат сохраняется в кэш-файл для использования в будущих запросах. Если аутентификация не удалась, кэш-файл удаляется.
4. **Создание результата**: После успешной аутентификации вызывается метод `create_authed` для создания результата запроса с использованием полученных данных аутентификации.
5. **Обработка результата**: Результат возвращается в виде асинхронного генератора, который может быть использован для стриминга данных.
