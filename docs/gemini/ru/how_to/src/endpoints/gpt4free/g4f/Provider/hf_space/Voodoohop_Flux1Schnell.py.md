...
2. **Предоставление параметров**: Передайте необходимые параметры в метод `create_async_generator()`, такие как:
    ...
    - `seed`: Случайное число, Используетсяое для генерации (по умолчанию 0).
...
4. **Обработка результатов**: Итерируйте по генератору, чтобы получить ссылки на сгенерированные изображения. Используй эти ссылки для загрузки или отображения изображений.
...
async def generate_image():
    ...
    async for image_response in generator:
        ...
        # Используй URL для загрузки или отображения изображения
