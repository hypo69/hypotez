...
В этом примере мы создание экземпляр `ChatGptt` и отправляем запрос с текстом "Привет! Как дела?". В цикле `async for` мы получаем каждый элемент потока данных от API и выводим текст ответа.