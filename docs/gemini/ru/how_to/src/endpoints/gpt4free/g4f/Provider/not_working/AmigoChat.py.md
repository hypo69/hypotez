...
2. **Выбор модели**:  
    -  Используй `chat_models` для получения списка доступных моделей для генерации текста. 
    - Используй `image_models` для получения списка доступных моделей для генерации изображений.
    -  Используй `model_aliases` для получения  альтернативных названий моделей.
3. **Генерация текста**: 
    -  Используй метод `create_async_generator` для запуска генерации текста.
