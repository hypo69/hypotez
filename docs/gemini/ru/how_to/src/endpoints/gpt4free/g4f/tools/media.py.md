...
Этот пример демонстрирует, как использовать функции `render_messages` и `merge_media` для отрисовки чата с медиа-файлами. Сначала мы создание список сообщений и список медиа-файлов. Затем мы объединяем медиа-файлы с сообщениями с помощью `merge_media`. Наконец, мы Используется `render_messages` для преобразования сообщений в формат, подходящий для отрисовки чата.