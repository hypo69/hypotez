...
2. **Получение моделей**: Используй метод `get_models` для получения списка доступных моделей Gemini.
3. **Создание асинхронного генератора**: Используй метод `create_async_generator` для создания асинхронного генератора, который будет отправлять запросы к API и получать ответы.
