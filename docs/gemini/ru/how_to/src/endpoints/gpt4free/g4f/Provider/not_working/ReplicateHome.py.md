...
В этом примере мы Используется `ReplicateHome` для генерации текста с помощью модели `gemma-2b`.  `create_async_generator` возвращает асинхронный генератор, который выдает фрагменты текста по мере их получения.
