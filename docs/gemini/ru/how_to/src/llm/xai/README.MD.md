...
Шаги выполнения
-------------------------
1. **Инициализация:** создание объект класса `XAI`, передавая в качестве аргумента свой API ключ.
2. **Создание запроса:**  создание список `messages`, представляющий диалог с моделью. Каждый элемент списка — словарь с ключами `role` (роль отправителя — `system`, `user`, `assistant`) и `content` (текст сообщения).
