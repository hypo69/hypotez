...
2. **Взаимодействие с веб-страницами:**
    - Используй методы `driver` для перехода по URL (`get_url`), прокрутки (`scroll`), ожидания (`wait`), получения HTML (`fetch_html`) и т.д.
3. **Взаимодействие с элементами (`ExecuteLocator`):**
    ...
    - Используй `driver.execute_locator` (асинхронный метод) для поиска элемента и выполнения действия (клик, ввод текста, получение атрибута, скриншот и т.д.).
...
5. **Логирование:**
    - Используй логгер (`src/logger/logger.py`) для отслеживания процесса работы, предупреждений и ошибок.
