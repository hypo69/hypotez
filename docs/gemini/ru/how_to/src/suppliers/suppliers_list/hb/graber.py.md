...
graber.description_short()  # Используется функцию для получения значения поля

# Получение значения цены
graber.price(value=100.00)  # Передача значение цены
```

В этом примере мы создание экземпляр класса `Graber`, передавая ему объект драйвера и индекс языка. Затем мы Используется функции `description_short` и `price` для получения значений полей. 
