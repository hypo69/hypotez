...
2. **Использование исключений:** В вашем коде используйте соответствующие исключения для обработки ошибок:
   ...
   * **InvalidArgumentException**: Используй это исключение, если аргументы функции некорректны.

   * **ProductIdNotFoundException**: Используй это исключение, если не найден идентификатор товара.

   * **ApiRequestException**: Используй это исключение, если запрос к AliExpress API не удался.

   * **ApiRequestResponseException**: Используй это исключение, если ответ API не соответствует ожидаемому формату.

   * **ProductsNotFoudException**: Используй это исключение, если не найдены товары.

   * **CategoriesNotFoudException**: Используй это исключение, если не найдены категории.

   * **InvalidTrackingIdException**: Используй это исключение, если идентификатор отслеживания отсутствует или неверен.

3. **Обработка исключений:** Используй оператор `try-except` для обработки исключений, возникающих во время работы с AliExpress API:
