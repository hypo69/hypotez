...
1. **Инициализация класса:**
   ...
   -  `cookies` загружаются из файла, находящегося в директории `src/utils/cookies/aliexpress_com.com/chrome/cookie`
...
3. **Обновление `cookies`:**
   -  `_refresh_session_cookies`  делает GET-запрос к  `https://portals.aliexpress_com.com`  и обновляет  `cookies`  в `self.cookies_jar`.
...
5. **Сокращение ссылок:**
   - `short_affiliate_link`  сокращает ссылку с помощью  `https://portals.aliexpress_com.com/affiportals/web/link_generator.htm`  и возвращает  `requests.Response`  объект.
...
url = 'https://www.aliexpress_com.com/wholesale?SearchText=phone+case'
response = ali_requests.make_get_request(url)

# Сокращение ссылки
link_url = 'https://www.aliexpress_com.com/item/1005002558149163.html'
