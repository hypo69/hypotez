...
2. **Получение данных о товарах:**
    - Используй метод `retrieve_product_details_as_dict(product_ids)` для получения информации о товарах по их ID.
...
3. **Генерация партнерских ссылок:**
    - Используй метод `get_affiliate_links(links, link_type)` для генерации партнерских ссылок для заданных товаров.
