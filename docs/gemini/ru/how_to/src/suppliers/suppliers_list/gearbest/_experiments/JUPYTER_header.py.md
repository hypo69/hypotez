...
```python
    ...
    # Используется объект `supplier` для дальнейшей работы
    print(supplier) 
```

### Изменения: 
- Заменены расплывчатые термины типа "получаем", "делаем" на более конкретные, например, "Определение", "добавляем", "Инициализация". 
