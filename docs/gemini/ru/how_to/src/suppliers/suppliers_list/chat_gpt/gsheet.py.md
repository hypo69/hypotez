...
3. **Обновление листа чата**: Используй метод `update_chat_worksheet()` для записи данных о разговоре в лист Google Sheets. 
...
5. **Установка листа категории**: Используй метод `set_category_worksheet()` для записи данных о категории в лист `'category'`.
...
6. **Получение листа категории**: Метод `get_category_worksheet()` извлекает данные о категории из листа `'category'`.
    - Возвращает данные в формате `SimpleNamespace`.
7. **Установка листа категорий**: Используй метод `set_categories_worksheet()` для записи данных о нескольких категориях в лист `'categories'`.
    - В качестве аргумента передайте данные в формате `SimpleNamespace`.
...
9. **Установка листа товара**: Используй метод `set_product_worksheet()` для записи данных о товаре в новый лист Google Sheets.
...
10. **Получение листа товара**: Метод `get_product_worksheet()` извлекает данные о товаре из листа `'products'`.
    - Возвращает данные в формате `SimpleNamespace`.
11. **Установка листа товаров**: Используй метод `set_products_worksheet()` для записи данных о нескольких товарах в лист `'products'`.
