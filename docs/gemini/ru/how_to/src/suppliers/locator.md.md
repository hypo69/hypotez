...
2. **Использование локатора**:  
   ...
   - Получите результат.
   - Используй полученный результат для дальнейшей работы.
...
- **`locator_description`**:  
  Это ключевой аспект, который делает локаторы более понятными и упрощает их использование. Используй `locator_description` для предоставления четкой информации о том, что делает каждый локатор.
- **Сложные локаторы**:  
