...
1. **Импортируйте модуль и создайте объект `BS`**:
   - Используй `from src.webdriver.bs import BS` для импорта модуля.
...
3. **Получите HTML-контент**:
   - Используй метод `get_url` для получения HTML-контента из файла или URL-адреса.
   -  ```python
...
5. **Выполните локатор**:
   - Используй метод `execute_locator` для выполнения локатора XPath и извлечения соответствующих элементов.
   -  ```python
