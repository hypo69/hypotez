...
Описание
-------------------------
Локаторы — это структурированные описания элементов на HTML-странице, Используетсяые для их поиска и взаимодействия с помощью веб-драйвера. Они представляют собой JSON-объекты, содержащие информацию о стратегии поиска, селекторе, атрибутах, действиях и других параметрах, необходимых для работы веб-драйвера.
...
2. **Загрузка локатора**:  Используй функцию `j_loads_ns` для загрузки локатора из JSON-файла.
...
class KspGraber(Graber):
    ...
    async def grab_page_async(self, *args, **kwargs) -> ProductFields:
        ...
        if 'ksp.co.il/mob' in self.driver.current_url:
            ...
            # Провера, что используются десктопные (если не были загружены ранее или переключились с мобильной)
