...
В этом примере мы создание экземпляр `CrawleePython` с максимальным количеством запросов 5, в режиме без графического интерфейса (`headless`), с использованием Firefox и опцией `--headless`. Затем мы запускаем ползучок с URL-адресом `https://www.example.com`.