...
В этом примере мы создание объект `driver` с помощью `Driver(Firefox)`, что инициализирует Firefox WebDriver. Затем можно использовать этот объект для выполнения действий в браузере Firefox. 
...
```
В данном примере мы Используется `driver.execute_locator()` чтобы выполнить действие "close_banner", которое закрывает pop-up окно.
