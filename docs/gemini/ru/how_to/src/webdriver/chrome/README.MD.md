...
3. **Открытие веб-страницы**: Используй метод `get()` экземпляра `Chrome` для открытия веб-страницы.
...
5. **Закрытие браузера**: Используй метод `quit()` экземпляра `Chrome` для закрытия браузера.
...
- **headers**: Настраиваемые HTTP-заголовки, Используетсяые в запросах браузера.
