...
2. **Вызов метода `execute_locator`**: Используй метод `execute_locator` для выполнения действий с элементом, указанным в локаторе. 
...
5. **Дополнительные параметры**: Используй `timeout_for_event` для указания условия ожидания, `message` для действий,  `typing_speed` для скорости ввода.
