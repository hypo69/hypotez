...
Описание
-------------------------
Данный блок кода представляет собой класс `Driver`, который обеспечивает унифицированный интерфейс для работы с веб-драйверами Selenium. 

Он предоставляет функциональность для:
...
Шаги выполнения
-------------------------
1. **Инициализация**: Создайте экземпляр класса `Driver` и передайте в качестве аргумента класс WebDriver (например, `Chrome` или `Firefox`).
2. **Переход по URL**: Используйте метод `get_url(url: str)` для перехода по указанному URL. 
3. **Управление куки**: Метод `_save_cookies_localy()`  сохраняет куки драйвера в локальный файл. 
4. **Прокрутка**: Метод `scroll(scrolls: int = 1, frame_size: int = 600, direction: str = 'both', delay: float = .3)` позволяет прокручивать страницу.
5. **Определение языка**: Метод `locale` возвращает язык страницы.
...
driver = Driver(Chrome, executable_path='/path/to/chromedriver')  # Инициализация драйвера Chrome

# Переход по URL
driver.get_url('https://www.example.com') 

# Прокрутка
driver.scroll(scrolls=3, direction='down')  # Прокрутка вниз на 3 шага
