...
2. **Проверка драйвера**: 
    - Используй метод `_check_driver()`, чтобы убедиться, что драйвер браузера успешно инициализирован.
3. **Поиск в интернете**: 
    - Используй метод `search(query: str, search_engine_url: str = "https://www.google.com")`, чтобы выполнить поиск в указанной поисковой системе.
