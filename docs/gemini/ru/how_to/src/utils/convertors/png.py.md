...
```
Как использовать класс `TextToImageGenerator` и функцию `webp2png`
=========================================================================================

Описание
-------------------------
Этот блок кода предоставляет функциональность для генерации PNG-изображений из текста и конвертации изображений из формата WEBP в формат PNG. Класс `TextToImageGenerator` позволяет создавать изображения с текстом, настраивая размер холста, шрифт, цвет фона и цвет текста. Функция `webp2png` конвертирует изображение из формата WEBP в PNG.

Шаги выполнения
-------------------------

1.  **Инициализация `TextToImageGenerator`**:
    *   Создание экземпляра класса `TextToImageGenerator` для использования его методов.

2.  **Генерация изображений с текстом**:
    *   Вызов асинхронного метода `generate_images` для создания PNG-изображений на основе списка строк.
    *   Настройка параметров, таких как директория вывода, шрифт, размер холста, отступы, цвет фона и цвет текста.
    *   Сохранение сгенерированных изображений в указанную директорию.

3.  **Конвертация WEBP в PNG**:
    *   Вызов функции `webp2png` с указанием путей к исходному WEBP-файлу и целевому PNG-файлу.
    *   Функция открывает WEBP-изображение, конвертирует его в PNG и сохраняет по указанному пути.
...
# Пример использования TextToImageGenerator
import asyncio
async def main():
    generator = TextToImageGenerator()
...
# Пример использования webp2png
    ...
    if webp2png(webp_path, png_path):
        print(f"Изображение {webp_path} успешно преобразовано в {png_path}")
    else:
        print(f"Не удалось преобразовать изображение {webp_path} в {png_path}")
asyncio.run(main())