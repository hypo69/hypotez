...
5. **Используй методы логирования для записи сообщений**:
   - Используй методы `info`, `success`, `warning`, `debug`, `error`, `critical` для записи сообщений с разными уровнями важности.
   - Пример: `logger.info('Это информационное сообщение')`

6. **Настройте цвета для консольного вывода (опционально)**:
   - Используй параметр `colors` при вызове методов логирования, чтобы задать цвет текста и фона.
