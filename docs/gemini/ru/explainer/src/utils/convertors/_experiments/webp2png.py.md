...
*   **src.utils.file.py**: Содержит функцию `get_filenames`, Используетсяую для получения списка файлов WebP в указанной директории.
...
*   `convert_images(webp_dir: Path, png_dir: Path) -> None`:
    ...
    *   Возвратое значение: `None` (функция ничего не возвращает).
