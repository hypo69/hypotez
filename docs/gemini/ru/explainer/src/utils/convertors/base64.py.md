...
-   **`base64_to_tmpfile(content: str, file_name: str) -> str`**:
    -   **Аргументы**:
        ...
        -   `file_name` (str): Имя файла, Используетсяое для определения расширения временного файла.
    -   **Возвратое значение**:
...
-   **`_`**: Используется для игнорирования первой части, Возвратой `os.path.splitext` (тип: str).
