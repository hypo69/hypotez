...
- `save_image_from_url_async(image_url: str, filename: Union[str, Path]) -> Optional[str]`:
  ...
  - **Возвратое значение:** Путь к сохраненному файлу (str) или None в случае ошибки.
...
- `get_image_bytes(image_path: Path, raw: bool = True) -> Optional[BytesIO | bytes]`:
  ...
  - **Возвратое значение:** BytesIO объект или байты изображения в формате JPEG, или None в случае ошибки.
...
- `get_raw_image_data(file_name: Union[str, Path]) -> Optional[bytes]`:
  ...
  - **Возвратое значение:** Бинарные данные файла или None, если файл не существует или произошла ошибка.
