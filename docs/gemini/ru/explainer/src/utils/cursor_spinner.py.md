...
-   `spinning_cursor()`:
    ...
    -   Возвратое значение: Генератор, который последовательно выдает символы `|`, `/`, `-`, `\`.
...
-   `show_spinner(duration: float = 5.0, delay: float = 0.1)`:
    ...
    -   Возвратое значение: Отсутствует (None).
...
-   `spinner` (generator): Экземпляр генератора `spinning_cursor`, Используетсяый в функции `show_spinner` для получения символов спиннера.
