...
-   `src.utils.jjson.j_loads_ns`: Локальный модуль, Используетсяый для загрузки JSON-файлов в виде объектов SimpleNamespace.
-   `src.logger.logger.logger`: Локальный модуль, Используетсяый для логирования событий.
...
-   `_set_launch_options(self, user_agent: Optional[str] = None, options: Optional[List[str]] = None) -> Dict[str, Any]`:
    ...
    -   Возвратое значение: Словарь с опциями запуска Playwright.
