...
2.  **Добавление docstring**:
    - Добавить docstring к каждой функции и методу, описывая их назначение, аргументы, Возвратые значения и возможные исключения. Использовать формат, принятый в `hypotez`.
