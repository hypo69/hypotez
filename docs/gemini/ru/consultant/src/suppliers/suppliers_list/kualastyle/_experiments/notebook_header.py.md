...
3.  **Удалить неИспользуетсяые импорты.** Это улучшит читаемость кода.
...
6.  **Явно указать Возвратое значение функции `start_supplier`.** Функция должна возвращать экземпляр класса `Supplier`.
...
9.  **Добавить docstring для функции `start_supplier`.** Описать назначение функции, параметры и Возвратое значение.
