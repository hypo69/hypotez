...
1.  **Документация**:
    - Добавить подробные docstring для класса `Graber`, метода `__init__`, включая описание параметров, Возвратых значений и возможных исключений.
    - Описать назначение каждого атрибута класса, например, `supplier_prefix`.

2.  **Декоратор**:
    - Раскомментировать и доработать декоратор `@close_pop_up`, если он необходим. В противном случае, удалить неИспользуетсяый код.
...
class Graber(Grbr):
    ...
    def __init__(self, driver: Optional[Driver] = None, lang_index: Optional[int] = None) -> None:
        ...
        Args:
            ...
            lang_index (Optional[int]): Индекс языка, Используетсяого на сайте.
        """
