...
- **Минусы**:
    ...
    - Функция `start_supplier` возвращает неИспользуетсяый объект `Supplier`.
...
3. **Удалить лишние импорты**:
    - Убрать неИспользуетсяые импорты, чтобы уменьшить зависимость кода.
