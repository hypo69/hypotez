...
-   В комментариях избегай использования местоимений, таких как *«делаем»*, *«переходим»*, *«Возврат»*, *«отправляем»* и т. д.. Вместо этого используй точные термины, такие как *«извлечение»*, *«проверка»*, *«выполнение»*, *«замена»*, *«вызов»*, *«Функция выполняет»*,*«Функция изменяет значение»*, *«Функция вызывает»*,*«отправка»*
