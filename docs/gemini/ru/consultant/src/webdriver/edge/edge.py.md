...
1. **Документация**:
   - Добавить подробные docstring для методов `_payload` и `set_options`, описывающие их назначение, параметры и Возвратые значения.
...
class Edge(WebDriver):
    ...
    Attributes:
        driver_name (str): Имя Используетсяого WebDriver, по умолчанию 'edge'.
    """
