...
Модуль собирает файл `summary.md` для компиляции средствами `mdbook`. Подробнее: https://chatgpt.com/share/6742f054-aaa0-800d-9f84-0ab035a2a2c2

**Качество кода:**

- **Соответствие стандартам**: 7/10
- **Плюсы**:
    - Четкая структура, разбиение на функции.
...
1.  **Добавить логирование**:
    ...
    - Заменить `print` на `logger.info` или `logger.error`.

2.  **Улучшить документацию**:
    - Добавить более подробные docstring для всех функций, включая описание параметров и возвращаемых значений.
...
5.  **Обработка исключений**:
    - Добавить логирование исключений с использованием `logger.error` и передачей информации об исключении.

6.  **Общая структура модуля**:
    - Добавить заголовок модуля с использованием формата, описанного в инструкции.
...
Модуль `make_summary` создает файл `SUMMARY.md`, который используется для компиляции документации с помощью `mdbook`.
Он рекурсивно обходит указанную директорию, фильтрует файлы по языку и генерирует структуру файла `SUMMARY.md`.
