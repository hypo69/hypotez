...
## \file /src/endpoints/prestashop/api/api_async.py

Модуль содержит асинхронный класс `PrestaShopAsync`, предназначенный для взаимодействия с API PrestaShop. Класс предоставляет методы для выполнения CRUD операций, поиска и загрузки изображений.
...
- **Минусы**:
    ...
    - Есть участки кода, закомментированные для отладки, которые следует удалить или заменить логированием.
    - Не все переменные аннотированы типами.
    - Использования `Union` вместо `|`
...
1.  **Документация**:
    ...
    *   Улучшить описание возвращаемых значений и возможных исключений.

2.  **Логирование**:
    *   Добавить `exc_info=True` при логировании ошибок для получения более подробной информации.
    *   Удалить или заменить отладочный код с закомментированными участками логированием.
...
6.  **Использование `|` вместо `Union`**:
    *   Заменить `Union[str, int]` на `str | int`.
...
class PrestaShopAsync:
    """Асинхронный класс для взаимодействия с API PrestaShop, использующий JSON и XML.

    Этот класс предоставляет асинхронные методы для взаимодействия с API PrestaShop,
    обеспечивая CRUD операции, поиск и загрузку изображений. Также предоставляет
    ...
    def _parse_response_error(
        ...
        """Разбор ошибки ответа от API PrestaShop асинхронно.
    ...
    async def search(
        self, resource: str, filter: Optional[str | dict] = None, **kwargs
    ) -> List[dict]:
    ...
    async def get_apis(self) -> Optional[dict]:
        ...
        """
        return await self._exec(resource='apis', method='GET', io_format=self.data_format)
    ...
    async def get_languages_schema(self) -> Optional[dict]:
        ...
        """
        try:
            response = await self._exec(resource='languages', display='full', io_format='JSON')
            return response
        except Exception as ex:
    ...
    async def get_product_images(self, product_id: int) -> Optional[dict]:
        ...
        """
        return await self._exec(
            f'products/{product_id}/images', method='GET', io_format=self.data_format
        )