...
4.  **Добавить логирование** с использованием модуля `src.logger`.
5.  **Оформить модуль в соответствии с руководством по оформлению кода PEP8**.
...
def clean_empty_cdata(xml_string: str) -> str:
    ...
    try:
        ...
        logger.error('Error while cleaning XML', ex, exc_info=True)
        return ''
...
def save_xml(xml_string: str, file_path: str) -> None:
    ...
    try:
        ...
        logger.error('Error while saving XML', ex, exc_info=True)
