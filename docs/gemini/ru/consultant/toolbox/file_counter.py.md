...
1. **Добавить заголовок модуля и документацию для всех функций и классов.**
    - Описать назначение модуля, параметры функций, Возвратые значения и возможные исключения.
2. **Использовать аннотации типов для переменных и параметров функций.**
    - Это улучшит читаемость и облегчит отладку кода.
3. **Удалить неИспользуетсяую переменную `MODE`.**
...
def count_lines_in_files(directory: Path) -> Tuple[int, int, int]:
    ...
    try:
        for filename in os.listdir(directory):
            ...
            if filepath.is_file():
                # Проверка, является ли файл текстовым, не находится ли он в директории __pycache__, и не является ли файлом Jupyter Notebook
...
def count_lines_classes_functions(file) -> Tuple[int, int, int]:
    ...
    for line in file:
        ...
        if line:  # Проверка, не является ли строка пустой
            lines += 1
