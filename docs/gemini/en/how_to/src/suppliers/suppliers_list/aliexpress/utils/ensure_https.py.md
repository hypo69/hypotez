...
3. **ensure_https_single function**: This function checks if the URL string contains the `https://` prefix. If not, it adds it. If the URL is a product ID, it builds a full URL by concatenating the `https://www.aliexpress_com.com/item/` prefix with the product ID and `.html` extension. 
...
```python
    ...
    print(https_url)  # Output: https://www.aliexpress_com.com/item/example_product_id.html
    ...
    print(https_urls)  # Output: ['https://www.aliexpress_com.com/item/example_product_id1.html', 'https://www.aliexpress_com.com/item/example_product_id2.html']
```