...
- **Безопасность:** 
    - Не храните учетные данные (логин, пароль) непосредственно в коде. Используй переменные окружения (например, `os.environ`).
- **Обработка ошибок:** 
...
- **Словарь _connection:** Не храните учетные данные (логин, пароль) напрямую в коде. Используй переменные окружения (например, `os.environ`).
