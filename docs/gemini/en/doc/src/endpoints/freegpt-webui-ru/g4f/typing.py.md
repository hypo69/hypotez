...
Этот модуль предоставляет типы данных, Используетсяые в `g4f` для типизации данных, передаваемых в функции и методы.

## Детали

Модуль `typing.py` определяет типы данных, которые используются для типизации параметров, Возвратых значений и атрибутов в `g4f`. 
