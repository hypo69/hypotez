...
Модуль предоставляет набор классов, которые соответствуют различным структурам данных, Возвратым GPT-4Free API. Эти классы используют библиотеку `pydantic` для проверки типов и валидации данных, что обеспечивает правильность и надежность данных, получаемых от API.
