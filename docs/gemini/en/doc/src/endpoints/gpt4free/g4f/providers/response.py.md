...
Модуль `response` в рамках проекта `hypotez` предназначен для обработки и форматирования различных типов ответов от моделей искусственного интеллекта, Используетсяых в проекте. В нем реализованы классы, которые представляют собой различные типы ответов, включая:
