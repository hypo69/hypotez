...
Этот модуль предназначен для взаимодействия с веб-сервисами, которые используют механизм защиты от ботов с использованием файлов HAR. HAR (HTTP Archive) - это стандартный формат, Используетсяый для записи и анализа трафика HTTP.
...
- `arkCookies` (dict): Словарь с куки, Используетсяыми при запросе к серверу Arkose.
- `userAgent` (str): Строка User-Agent, Используетсяая при запросе к серверу Arkose.
