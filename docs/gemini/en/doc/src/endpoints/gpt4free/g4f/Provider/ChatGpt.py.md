...
**Attributes**:
- `label` (str):  Метка, Используетсяая для идентификации провайдера.
...
- `default_model` (str):  Модель по умолчанию, Используетсяая провайдером.
...
**Parameters**:
- `user_agent` (str):  Строка user-agent, Используетсяая для идентификации браузера.
...
- `pow_req` (str):  Токен, Используетсяый для получения данных.
...
**Parameters**:
- `seed` (str):  Ключ, Используетсяый для генерации токена.
