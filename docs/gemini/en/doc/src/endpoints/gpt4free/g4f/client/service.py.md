...
Этот модуль отвечает за определение модели и провайдера, Используетсяых для обработки запросов к API GPT-4 Free. Он содержит функции, которые позволяют пользователю явно указать модель и провайдера или автоматически выбрать наиболее подходящие.
