...
**Attributes**:
    ...
    - `_access_token (str)`: Токен доступа, Используетсяый для аутентификации в API.
    - `_cookie_jar`: Хранит cookie-файлы, Используетсяые для сессии.
...
## Parameter Details
    ...
    - `proxy (str)`: Прокси-сервер, Используетсяый для обхода блокировок или 
