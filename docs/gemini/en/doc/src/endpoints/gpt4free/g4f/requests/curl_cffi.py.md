...
**Описание**: Класс для создания форм данных, Используетсяых в HTTP запросах.
...
- `session (StreamSession)`: Сессия, Используетсяая для соединения.
...
- Используй `logger` из модуля `src.logger` для логирования.
