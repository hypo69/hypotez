...
## Детали
В этом модуле мы Используется `Client` класс из `g4f` библиотеки для взаимодействия с `GPT-4Free` API. 
Мы Установка соединение с `HuggingFaceMedia` провайдером, используя свой ключ API, и получаем список доступных моделей для генерации видео.
