...
Этот модуль содержит набор юнит-тестов для различных функций, Используетсяых в проекте `tinytroupe`. Тесты охватывают функции из модуля `tinytroupe.utils`, а также декоратор `llm`. 
