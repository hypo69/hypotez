...
Модуль `ToolBox_DataBase` предоставляет инструменты для работы с базой данных SQLite, Используетсяой в проекте. Он содержит класс `DataBase`, который позволяет создавать, обновлять и извлекать данные из таблицы базы данных.
