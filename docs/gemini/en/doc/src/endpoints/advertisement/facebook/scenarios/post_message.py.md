...
**Параметры**:
- `d` (Driver): Экземпляр драйвера, Используетсяого для взаимодействия с веб-страницей.
...
**Параметры**:
- `d` (Driver): Экземпляр драйвера, Используетсяого для взаимодействия с веб-страницей.
- `category` (SimpleNamespace): Детали категории, Используетсяые для заголовка и описания сообщения.
...
**Параметры**:
- `d` (Driver): Экземпляр драйвера, Используетсяого для взаимодействия с веб-страницей.
- `message` (SimpleNamespace): Детали сообщения, Используетсяые для заголовка и описания сообщения.
- `no_video` (bool, optional): Если `True`, не загружать видеофайлы. По умолчанию `False`.
