# Определение локаторов полей на HTML-странице

В этом документе описывается формат и назначение локаторов полей, Используетсяых для взаимодействия с элементами на HTML-странице в контексте проекта `hypotez`.
...
"close_banner": {
  ...
  "additional_images_urls": {
    ...
    "timeout_for_event": "presence_of_element_located",
    "event": null,
    "text_to_be_present_in_element":"","locator_description": "Получаю список `urls` для дополнительных изображений."
  },
  "id_supplier": {
    "attribute": "innerText",
    "by": "XPATH",
  ...
  "default_image_url": {
    ...
    "event": "screenshot()",
    "mandatory": true,
    "text_to_be_present_in_element":"","locator_description": "Внимание! В Morlevi изображение получается через screenshot и возвращается в виде PNG (`bytes`)."
  }
```
