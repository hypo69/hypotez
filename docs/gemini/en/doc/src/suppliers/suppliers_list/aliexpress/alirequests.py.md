...
- `webdriver_for_cookies` (str, optional): Имя веб-драйвера, Используетсяого для загрузки куков. По умолчанию - "chrome".
...
1. **Инициализация**: Создается объект `AliRequests` с указанием веб-драйвера, Используетсяого для загрузки куков. 
...
- **Описание**: Имя веб-драйвера, Используетсяого для загрузки куков.
- **Тип**: str
