...
Этот модуль содержит класс `Currency`, который представляет собой набор валют, Используетсяых в API AliExpress. Он предоставляет атрибуты для каждой валюты, которые могут быть использованы в других модулях проекта. 
