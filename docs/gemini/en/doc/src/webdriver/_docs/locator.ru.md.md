...
Этот документ описывает локаторы, Используетсяые в проекте `hypotez` для взаимодействия с веб-элементами на страницах. Локаторы — это конфигурационные объекты, которые описывают, как найти и взаимодействовать с веб-элементами. Они передаются в класс `ExecuteLocator` для выполнения различных действий, таких как клики, отправка сообщений, извлечение атрибутов и т. д. 
