...
Модуль `claude.py` реализует функциональность для взаимодействия с API модели Claude от Anthropic. Используй класс `ClaudeClient` для выполнения различных задач, таких как генерация текста, анализ тональности и перевод.