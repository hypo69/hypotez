# x.ai — искусственный интеллект от Илона Маска

## Обзор

Этот файл содержит обзор x.ai — компании, основанной Эллоном Маском, которая занимается разработкой искусственного интеллекта (ИИ) для различных приложений. 

## Детали

x.ai была запущена в 2023 году и находится в стадии активного развития. Хотя детали ее товаров и технологий могут быть не полностью раскрыты, некоторые ключевые моменты можно выделить:
...
- **Неизвестные детали**: Пока что публично доступная информация о конкретных товарах x.ai ограничена. Однако, учитывая опыт Маска и его команды, можно ожидать, что товары будут иметь революционный потенциал.
...
xAI запустил Grok в прошлом году, но он был доступен только подписчикам Premium. В августе стартап добавил в чат-бот функцию перевода текста в изображение, а в октябре —опцию распознавания картинок. Тогда же стартап открыл API Grok для разработчиков. 
