...
Этот модуль предназначен для автоматизации браузера с использованием Playwright. Он упрощает настройку и управление браузером, позволяя разработчикам сосредоточиться на логике своих задач, а не на деталях конфигурации.
