...
Этот модуль предоставляет пользовательскую реализацию `PlaywrightCrawler` с использованием библиотеки Playwright. Он позволяет настраивать параметры запуска браузера, такие как user-agent, настройки прокси, размер области просмотра и другие опции, определенные в файле `playwrid.json`.

## Подробнее

Этот модуль предназначен для упрощения и централизации конфигурации Playwright Crawler, обеспечивая гибкость настройки браузера и удобство управления параметрами через файл конфигурации. Он также предоставляет расширенные возможности логирования и обработки ошибок для облегчения отладки и поддержки.
...
Класс WebDriver использует `logger` из `src.logger` для логирования ошибок, предупреждений и общей информации. Все проблемы, возникающие во время инициализации, конфигурации или выполнения, будут залогированы для облегчения отладки.
