...
# Принудительно Используется простую строку пути, которая *может* работать с регулярным выражением, если диск C: существует
...
if os.path.exists(Path(tmp_file_path)): # Убедитесь, что временный файл все еще существует
   if os.name == 'nt': # создание путь, ожидаемый регулярным выражением
