...
-   `url` (str): URL-адрес, Используетсяый по умолчанию для загрузки списка прокси-серверов.
-   `proxies_list_path` (Path): Путь к файлу, Используетсяый по умолчанию для сохранения загруженного списка прокси-серверов.