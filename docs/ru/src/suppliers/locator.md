...
Этот документ содержит описание формата и структуры локаторов, Используетсяых для поиска и взаимодействия с элементами на веб-странице. Локаторы представлены в формате `JSON` и используются для автоматизации действий веб-драйвера.
...
- [Описания ключей локаторов](#описания-ключей-локаторов)
...
"close_banner": {
    ...
    "event": "click()",
    "text_to_be_present_in_element":"","locator_description": "Закрытие всплывающего окна. Если оно не появляется — не проблема (`mandatory`: `false`)."
  },
  "additional_images_urls": {
    ...
    "timeout_for_event": "presence_of_element_located",
    "event": null,
    "text_to_be_present_in_element":"","locator_description": "Получение списка `url` для дополнительных изображений."
  },
  "id_supplier": {
    "attribute": "innerText",
    "by": "XPATH",
...
7. **`timeout_for_event`**:
   Время ожидания (в секундах) для события. `"presence_of_element_located"` означает, что WebDriver будет ждать появления элемента, прежде чем выполнять событие.
