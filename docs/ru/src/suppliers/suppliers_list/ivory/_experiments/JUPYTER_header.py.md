...
- `sys.path` (list): Список путей, Используетсяых Python для поиска модулей. В данный список добавляется корневая директория проекта, чтобы обеспечить доступ к модулям внутри проекта.
