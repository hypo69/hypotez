...
Модуль `arguments.py` содержит функции для обработки и валидации аргументов, Используетсяых в API AliExpress. Он предоставляет инструменты для преобразования входных данных в нужный формат, а также для проверки их корректности.
