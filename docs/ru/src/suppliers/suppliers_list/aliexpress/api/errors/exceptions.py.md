...
Этот модуль содержит пользовательские исключения, Используетсяые для обработки ошибок, связанных с API AliExpress. Все исключения наследуются от базового класса `AliexpressException`.
