# Модуль: src.suppliers.suppliers_list.aliexpress_com.api.models

## Обзор

Модуль определяет классы, представляющие различные параметры запросов к API AliExpress. Эти классы включают типы товаров, параметры сортировки и типы ссылок, Используетсяые при взаимодействии с API.

## Подробнее

Этот модуль предоставляет константы, Используетсяые для формирования запросов к API AliExpress. Он содержит классы `ProductType`, `SortBy` и `LinkType`, которые определяют возможные значения для соответствующих параметров запроса.
