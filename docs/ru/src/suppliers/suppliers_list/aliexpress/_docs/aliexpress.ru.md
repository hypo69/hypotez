...
**Описание**: Инициализирует класс `Aliexpress`. Функция создает экземпляры классов `Supplier`, `AliRequests` и `AliApi`, а также настраивает локаль и веб-драйвер в зависимости от переданных параметров.
...
- Функция `__init__` инициализирует класс `Aliexpress`.
- В зависимости от переданных параметров, настраивается использование вебдрайвера или режима запросов.
- Устанавливается локаль для работы с AliExpress.
- Создаются экземпляры классов `Supplier`, `AliRequests` и `AliApi` для выполнения запросов и обработки данных.
- Передаются дополнительные аргументы и параметры внутренним компонентам.
