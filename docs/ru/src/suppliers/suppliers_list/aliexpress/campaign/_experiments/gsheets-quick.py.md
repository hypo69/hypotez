...
- `gs` (AliCampaignGoogleSheet): Экземпляр класса AliCampaignGoogleSheet, Используетсяый для работы с Google Sheets.
