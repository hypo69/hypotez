...
Модуль предназначен для разбора XLS-таблиц, сгенерированных в личном кабинете portals.aliexpress_com.com, и извлечения информации об акциях.
...
- `src.suppliers.suppliers_list.aliexpress_com.DealsFromXLS`: Класс для парсинга XLS-файлов с данными об акциях AliExpress.
- `src.utils.printer.pprint`: Функция для "красивого" вывода данных в консоль.

## Переменные

- `deals_parser`: Экземпляр класса `DealsFromXLS`, Используетсяый для парсинга данных об акциях. Инициализируется с параметрами `language='EN'` и `currency='USD'`.
...
for deal in deals_parser.get_next_deal():
    ...
    ...
```
В этом примере `deals_parser` инициализируется классом `DealsFromXLS` с указанием языка (`EN`) и валюты (`USD`). Затем в цикле `for` происходит итерация по акциям, Возвратым методом `get_next_deal()`. Каждая акция выводится на экран с использованием функции `pprint`.