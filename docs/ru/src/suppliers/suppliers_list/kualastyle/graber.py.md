...
- `lang_index` (int): Индекс языка, Используетсяый для локализации контента.
...
- `supplier_prefix` (str): Префикс поставщика, Используетсяый для идентификации поставщика. В данном случае установлен в `'kualastyle'`.
```