...
**Параметры**:
- `supplier_prefix` (str, optional): Префикс поставщика, Используетсяый для определения конкретного поставщика (например, `'aliexpress'`). По умолчанию `'aliexpress'`.
- `locale` (str, optional): Локаль, Используетсяая для поставщика (например, `'en'` для английского). По умолчанию `'en'`.
