...
Модуль `ai_string_utils.py` предоставляет утилиты для обработки и нормализации текстовых данных, Используетсяых в задачах обучения моделей ИИ и обработки ответов от этих моделей. Он включает в себя функции для очистки строк для обучения и для удаления обрамляющих блоков кода из ответов ИИ.
