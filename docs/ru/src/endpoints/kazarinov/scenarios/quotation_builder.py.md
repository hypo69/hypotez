...
**Назначение**: Преобразует поля продукта из объекта `ProductFields` в словарь для использования в AI-модели.

**Параметры**:
- `f` (ProductFields): Объект, содержащий распарсенные данные о продукте.

**Возвращает**:
- `dict`: Словарь с отформатированными данными продукта. Возвращает пустой словарь, если `id_product` отсутствует.

**Как работает функция**:
1. Проверяет наличие `id_product` в данных продукта. Если отсутствует, возвращает пустой словарь.
2. Извлекает значения полей продукта, таких как `name`, `description`, `description_short` и `specification`.
3. Возвращает словарь, содержащий извлеченные данные продукта.
...
**Назначение**: Обрабатывает список продуктов с помощью AI-модели Google Gemini.
...
2. Формирует запрос для AI-модели, объединяя команду и список продуктов.
...
**Назначение**: Асинхронно обрабатывает список продуктов с помощью AI-модели Google Gemini.
...
**Назначение**: Сохраняет данные продукта в файл в формате JSON.
...
**Как работает функция**:
1. Формирует путь к файлу, где будут сохранены данные продукта.
