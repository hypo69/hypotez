...
- `DEFAULT_MESSAGES`: Список сообщений, Используетсяых в тестах. Содержит одно сообщение с ролью "user" и содержанием "Hello".
- `test_model`: Экземпляр класса `g4f.models.Model`, Используетсяый в тестах. Определяет модель с именем "test/test_model" и моковым провайдером `ModelProviderMock`.