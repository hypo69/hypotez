...
- `model` (g4f.models.default): Модель, Используетсяая для генерации текста.
- `messages` (List[Dict[str, str]]): Список сообщений, Используетсяых для генерации текста. Каждое сообщение содержит роль (`role`) и содержание (`content`).
