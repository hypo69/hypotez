...
-   `provider` (ProviderType): Базовый провайдер, Используетсяый для обработки задач, не связанных с созданием изображений.
...
-   `include_placeholder` (bool, optional): Флаг, указывающий, следует ли включать заполнитель (placeholder) для изображений в Возвратый результат. По умолчанию установлено значение `True`.
...
-   `model` (str): Имя модели, Используетсяой для генерации завершения.
...
-   `model` (str): Имя модели, Используетсяой для генерации ответа.
