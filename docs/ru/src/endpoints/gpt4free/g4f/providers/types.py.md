...
Модуль `types.py` содержит абстрактные базовые классы для определения провайдеров, Используетсяых в проекте `hypotez`. Он определяет интерфейсы и базовые атрибуты, необходимые для реализации различных провайдеров, таких как `BaseProvider` и `BaseRetryProvider`. Также содержит класс `Streaming`, Используетсяый для потоковой передачи данных.
