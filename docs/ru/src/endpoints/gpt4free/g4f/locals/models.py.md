...
Модуль предназначен для загрузки, форматирования, чтения и сохранения информации о моделях, Используетсяых в проекте `hypotez`. Он предоставляет функции для получения списка моделей с удаленного ресурса, их обработки и сохранения в локальном файле, а также для управления каталогом, где хранятся модели.
