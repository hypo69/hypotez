...
- `_system` (`Optional[str]`): Используетсяая система модели. По умолчанию `openai`.
...
**Параметры**:
- `model_name` (`str`): Имя Используетсяой модели AI. Список доступных имен моделей можно найти [здесь](https://github.com/openai/openai-python/blob/v1.54.3/src/openai/types/chat_model.py#L7).
...
- `system` (`str | None`, optional): Используетсяый провайдер модели. По умолчанию `'openai'`.
...
- Она также настраивает роль системного запроса и Используетсяую систему.
