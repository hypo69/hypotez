...
Этот модуль содержит классы и функции, необходимые для работы с инструментами, Используетсяыми в g4f. Он включает в себя:
...
*   `messages` (Messages): Список сообщений.
*   `tool` (dict): Словарь, содержащий информацию об инструменте.
*   `provider` (Any): Провайдер, Используетсяый для генерации.
...
*   `provider` (ProviderType): Провайдер, Используетсяый для генерации.
...
*   `model` (str): Имя модели.
*   `messages` (Messages): Список сообщений.
*   `provider` (Optional[str], optional): Провайдер, Используетсяый для генерации. По умолчанию `None`.
*   `tool_calls` (Optional[List[dict]], optional): Список вызовов инструментов. По умолчанию `None`.
*   `**kwargs`: Дополнительные аргументы.
