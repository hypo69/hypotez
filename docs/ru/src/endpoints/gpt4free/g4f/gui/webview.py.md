...
- `storage_path` (str, optional): Путь к директории, где `webview` будет хранить свои данные (например, кэш). Если не указан, используется директория по умолчанию, Определениеая с помощью `platformdirs`.
- `gui` (str, optional): <инструкция для модели gemini:необходимо уточнить, что это за параметр и какое влияние он оказывает на работу функции.> По умолчанию `None`.
