...
Тип `SHA256` представляет собой новый тип, основанный на строке, Используетсяый для хранения SHA256 хешей.
