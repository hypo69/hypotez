...
- `origin` (str): Полный URL домена, Используетсяый для формирования URL-адресов, значение `"https://lambda.chat"`.
...
- `default_model` (str): Модель, Используетсяая по умолчанию, значение `"deepseek-llama3.3-70b"`.
- `reasoning_model` (str): Модель, Используетсяая для логических рассуждений, значение `"deepseek-r1"`.
...
- `models` (list[str]): Копия списка `fallback_models`, Используетсяая для хранения доступных моделей.
- `model_aliases` (dict[str, str]): Словарь, содержащий псевдонимы для различных моделей.

**Принцип работы**:

Класс `LambdaChat` наследует от класса `HuggingChat` и переопределяет некоторые атрибуты, чтобы настроить взаимодействие с конкретной чат-моделью Lambda Chat. Он определяет доменное имя, URL, Используетсяые модели и другие параметры, необходимые для подключения и работы с сервисом.
...
- `reasoning_model` (str): Модель, Используетсяая для логических рассуждений.
