...
Модуль `models.py` определяет списки доступных моделей для различных задач, таких как обработка текста и изображений, Используетсяых в проекте `hypotez`. Он также предоставляет псевдонимы для упрощения обращения к моделям. Этот модуль является частью функциональности `gpt4free` и `g4f`.
