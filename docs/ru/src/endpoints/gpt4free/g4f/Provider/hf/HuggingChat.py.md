...
-   `models` (dict): Словарь, содержащий информацию о моделях, Используетсяых в беседе.
...
-   `default_vision_model` (str): Мультимодальная модель, Используетсяая по умолчанию.
