...
- `default_model` (str): Модель, Используетсяая по умолчанию для генерации текста (`gemini-1.5-pro`).
- `default_image_model` (str): Модель, Используетсяая по умолчанию для генерации изображений (`flux`).
...
- Результаты, Возвратые этими методами, передаются через `yield`, что делает функцию генератором.
...
- Сначала формируется Используетсяый промпт (`used_prompt`) с использованием функции `format_image_prompt`, которая принимает список сообщений (`messages`) и основной промпт (`prompt`).
...
- Если URL изображения получен, он передается в виде объекта `ImageResponse`, который содержит список URL изображений (`images`) и альтернативный текст (`alt`), в данном случае Используетсяый промпт.
