...
- `default_model` (str): Модель, Используетсяая по умолчанию, если не указана другая.

**Методы**:
- `create_async_generator()`: Создает асинхронный генератор для получения ответов от Koala API.
- `_parse_event_stream()`: Статический метод для парсинга потока событий, Возвратого API.
...
5. **Генерация данных**: Асинхронно перебирает чанки (фрагменты) из потока событий, Возвратого `_parse_event_stream`, и выдает их.
...
```python
    ...
    async def _parse_event_stream(response: ClientResponse) -> AsyncGenerator[Dict[str, Any], None]:
        """
        Разбирает поток событий, Возвратый API, и извлекает данные.
...
async def main():
    ...
    async with ClientSession() as session:
        # создание фиктивный объект response для примера
        class MockResponse:
