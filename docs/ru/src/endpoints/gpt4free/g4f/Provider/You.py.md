...
- `label` (str): Метка провайдера, Используетсяая для идентификации провайдера.
- `url` (str): URL сервиса You.com, Используетсяый для взаимодействия с API.
...
- `_cookies_used` (int): Счетчик использования cookies.
- `_telemetry_ids` (List[Any]): Список идентификаторов телеметрии, Используетсяых для отслеживания использования.
