...
- `default_model` (str): Модель, Используетсяая по умолчанию для текстовых запросов (="janus-pro-7b").
- `default_image_model` (str): Модель, Используетсяая по умолчанию для запросов изображений (="janus-pro-7b-image").
- `default_vision_model` (str): Модель, Используетсяая по умолчанию для vision запросов (=default_model).
...
- `session` (StreamSession): Асинхровая HTTP-сессия, Используетсяая для выполнения запроса.
...
- `seed` (int): Зерно для генерации случайных чисел, Используетсяое для воспроизводимости результатов.
...
- `seed` (int, optional): Зерно для генерации случайных чисел, Используетсяое для воспроизводимости результатов. По умолчанию `None`.
