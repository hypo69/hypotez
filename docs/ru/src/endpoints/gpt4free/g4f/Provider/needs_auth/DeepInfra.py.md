...
- `default_model` (str): Модель, Используетсяая по умолчанию для генерации текста.
- `default_image_model` (str): Модель, Используетсяая по умолчанию для генерации изображений.
