...
Модуль определяет соответствия между именами моделей, Используетсяыми в проекте `hypotez`, и именами, Используетсяыми в API TheB.AI. Класс `ThebApi` предоставляет методы для аутентификации и создания запросов к API.
...
- `default_model (str)`: Модель, Используетсяая по умолчанию ("theb-ai").
