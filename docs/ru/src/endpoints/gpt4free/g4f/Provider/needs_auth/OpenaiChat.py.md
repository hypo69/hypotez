...
- `default_image_model` (str): Модель для генерации изображений, Используетсяая по умолчанию.
...
**Как работает функция**:
Функция возвращает словарь заголовков по умолчанию, Используетсяых для запросов к API OpenAI.
