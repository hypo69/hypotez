...
- `default_model` (str): Модель, Используетсяая по умолчанию ("Copilot").
...
2. **Создание запроса**:
   ...
   - Он итерируется по частям ответа, Возвратым `create_completion`, и передает их вызывающей стороне.
