...
- `default_model` (str): Модель, Используетсяая по умолчанию ("llama-3-sonar-large-32k-online").
...
- `api_base` (str): Базовый URL для API запросов Perplexity AI.
- `default_model` (str): Имя модели, Используетсяой по умолчанию, если не указана другая.
- `models` (list): Список поддерживаемых моделей Perplexity AI.
