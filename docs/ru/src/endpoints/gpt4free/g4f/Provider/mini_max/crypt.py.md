...
Модуль `crypt.py` предназначен для реализации криптографических функций, Используетсяых в запросах к API mini_max. Он включает в себя функции для хеширования, генерации заголовков и подготовки тела запроса.
...
Модуль содержит функции для имитации криптографических операций, Используетсяых mini_max API, включая генерацию хешей, заголовков и тел запросов. Он также включает асинхронную функцию для получения callback из браузера.
...
- `page`(`Tab`): Объект, представляющий вкладку браузера, Используетсяый для выполнения JavaScript кода и взаимодействия с содержимым страницы.
