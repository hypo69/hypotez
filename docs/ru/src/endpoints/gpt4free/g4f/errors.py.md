...
Модуль содержит определения пользовательских исключений, Используетсяых в библиотеке `g4f` (gpt4free). Эти исключения предназначены для обработки различных ошибок, которые могут возникать во время работы с различными поставщиками (providers), моделями и API.
