...
**Назначение**:
Инициализирует экземпляр класса `CodeAssistant` с заданными параметрами, такими как роль, язык и Используетсяые модели.
...
```python
    async def _save_response(self, file_path: Path, response: str, model_name: str) -> bool:
        ...
        дополнительный суффикс, Определениеый ролью.
...
**Назначение**:
Сохраняет ответ модели в файл с добавлением суффикса, Определениеого ролью.
...
```python
    def _remove_outer_quotes(self, response: str) -> str:
        ...
        # Если строка начинается с '```python' или '```mermaid', Возврат её без изменений. Это годный код
