...
4.  **Сохранение ответов**: Сохраняет сгенерированную документацию в файлы Markdown в соответствующих директориях, Определениеых ролью (например, `doc_writer`).
...
*   `openai_model_name` (str): Имя Используетсяой модели OpenAI (`gpt-4o-mini`).
