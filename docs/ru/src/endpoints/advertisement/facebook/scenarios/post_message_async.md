...
*   `d` (Driver): Экземпляр `Driver`, Используетсяый для взаимодействия с веб-страницей.
...
*   `d` (Driver): Экземпляр `Driver`, Используетсяый для взаимодействия с веб-страницей.
*   `category` (SimpleNamespace): Объект с деталями категории, Используетсяыми для заголовка и описания поста.
