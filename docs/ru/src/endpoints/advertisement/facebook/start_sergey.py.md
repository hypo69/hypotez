...
*   `d` (Driver): Экземпляр драйвера веб-браузера, Используетсяый для взаимодействия с Facebook.
*   `promoter_name` (str): Имя рекламодателя, Используетсяое для идентификации кампании.
...
*   `currency` (str): Валюта, Используетсяая в рекламной кампании.
...
3.  Для каждой пары язык-валюта:
    ...
    *   Запускает кампании для aliexpress_com.
