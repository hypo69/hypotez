...
Модуль `config.py` предназначен для хранения и управления конфигурационными параметрами Telegram-бота, Используетсяого для цифрового рынка. Он определяет класс `Settings`, который загружает параметры из переменных окружения и предоставляет удобный доступ к ним.
...
- `dp`: Экземпляр класса `Dispatcher` из библиотеки `aiogram`, Используетсяый для обработки входящих сообщений и событий.
