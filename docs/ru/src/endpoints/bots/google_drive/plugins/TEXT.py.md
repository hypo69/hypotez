...
Модуль содержит текстовые константы и настройки, Используетсяые в боте для загрузки файлов на Google Drive. Он определяет имена папок, учетные данные для Mega, приветственные сообщения, справку, сообщения о статусе загрузки и другие параметры, необходимые для работы бота.
