...
- `skill` (str): Уровень экспертизы, Определениеый моделью (`'expert'` для `gpt-4`, `'intermediate'` в противном случае).
...
5.  **Отправка запроса и обработка ответа**:
    ...
    *   Функция `output` используется в качестве `content_callback` для обработки чанков данных, Возвратых API.
