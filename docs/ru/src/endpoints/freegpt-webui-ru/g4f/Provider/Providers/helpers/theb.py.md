...
4.  **Отправка запроса и обработка ответа**:
    ...
    *   Функция `format` используется в качестве callback-функции для обработки потоковых данных, Возвратых сервером. Эта функция извлекает и печатает контент из каждого чанка данных.
