...
**Описание**: Класс предназначен для определения наборов опций, Используетсяых при взаимодействии с Bing AI.
...
**Описание**: Класс, содержащий значения по умолчанию для различных параметров, Используетсяых при взаимодействии с Bing AI.

**Атрибуты**:
- `delimiter` (str): Разделитель, Используетсяый для разделения сообщений в потоке данных.
- `ip_address` (str): IP-адрес, Используетсяый для обхода географических ограничений.
...
def run(generator):
    ...
    Yields:
        Any: Значения, Возвратые генератором.
