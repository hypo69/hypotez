...
def _create_completion(model: str, messages: list, stream: bool, **kwargs):
    ...
    Args:
        model (str): Имя Используетсяой модели (в данном случае всегда `gpt-3.5-turbo`).
        messages (list): Список сообщений, Используетсяых для генерации текста. Каждое сообщение представляет собой словарь с ключом `content`, содержащим текст сообщения.
