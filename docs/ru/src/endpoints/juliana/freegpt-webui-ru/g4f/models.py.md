...
Этот модуль предоставляет структуру для определения и управления различными моделями, Используетсяыми в приложении. Каждая модель представлена классом внутри класса `Model`, что позволяет удобно группировать и идентифицировать модели. `ModelUtils` предоставляет механизм для динамического выбора класса модели на основе строкового идентификатора. Это позволяет упростить конфигурацию и выбор моделей во время выполнения.
