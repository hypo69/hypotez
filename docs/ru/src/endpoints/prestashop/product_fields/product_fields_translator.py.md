...
Модуль `product_fields_translator` предназначен для перевода полей товара на языки, Используетсяые в клиентской базе данных PrestaShop. Он обеспечивает соответствие между идентификаторами языков в системе поставщика и в системе клиента, а также позволяет обновлять и добавлять переводы товаров.
