...
-   `person_prompt_template_path` (str): Путь к шаблону mustache, Используетсяому для генерации запросов к модели.
-   `context_text` (str): Текст контекста, Используетсяый для генерации персонажей.
...
-   `generic_context_text` (str): Общий текст контекста, Используетсяый для генерации экземпляров `TinyPersonFactory`.
