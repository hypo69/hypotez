...
- `context_cache` (list): Список, Используетсяый для хранения контекстной информации между вызовами функции `enrich_content`. Это позволяет сохранять и использовать информацию о предыдущих запросах и ответах для улучшения качества обогащения.
