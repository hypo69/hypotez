...
- `cached_trace` (list): Список состояний симуляции, Используетсяых для кэширования.
...
```python
 def __init__(self, id="default", cached_trace:list=None):
    ...
    Args:
        ...
        cached_trace (list, optional): Список состояний симуляции, Используетсяых для кэширования. По умолчанию `None`.
    """
```
...
В конце функция инициализирует `cached_trace` списком состояний симуляции, Используетсяых для кэширования. Если `cached_trace` не передан в качестве аргумента, то инициализирует пустой список.
...
**Описание**: Исключение, Используетсяое для прерывания транзакции.
