...
Класс `ABRandomizer` позволяет инициализировать параметры A/B-теста, такие как имена контрольной и тестовой групп, а также имена, Используетсяые для представления этих групп пользователю. Это позволяет абстрагироваться от реальных названий вариантов и представлять их в более понятном виде. Также поддерживается список названий, которые не должны быть рандомизированы.
