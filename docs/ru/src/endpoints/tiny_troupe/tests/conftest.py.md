...
Этот файл определяет глобальные переменные и функции, Используетсяые в процессе тестирования. Он обрабатывает опции командной строки, переданные в `pytest`, и устанавливает соответствующие значения глобальных переменных, которые затем используются в тестах для определения поведения кэширования API и запуска примеров.
