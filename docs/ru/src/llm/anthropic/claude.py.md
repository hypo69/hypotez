...
**Атрибуты**:
- `client` (anthropic.Client): Клиент Anthropic, Используетсяый для выполнения запросов к API Claude.
...
**Параметры**:
- `api_key` (str): API-ключ, Используетсяый для аутентификации и авторизации при взаимодействии с API Claude.
