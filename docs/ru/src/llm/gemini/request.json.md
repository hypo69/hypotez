...
Данный файл (`request.json`) описывает структуру JSON-запроса, Используетсяого для взаимодействия с моделью Google Gemini. Он содержит параметры для настройки запроса, такие как контент, системные инструкции, инструменты, настройки безопасности, конфигурацию генерации и метки.
