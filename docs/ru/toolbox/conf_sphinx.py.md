...
Модуль `conf.py` используется для конфигурации генерации документации Sphinx для проекта `hypotez`. Он определяет настройки проекта, такие как имя, авторские права, версия релиза, а также указывает на Используетсяые расширения Sphinx, пути к шаблонам и статическим файлам. Кроме того, модуль содержит функции для фильтрации файлов, которые следует исключить из документации.
...
- `extensions` (list): Список расширений Sphinx, Используетсяых для генерации документации.
...
- `default_theme` (str): Тема оформления, Используетсяая по умолчанию.
...
3.  **Настройка Sphinx**: Определяются основные параметры Sphinx, такие как Используетсяые расширения, пути к шаблонам и статическим файлам, а также шаблоны исключений.
