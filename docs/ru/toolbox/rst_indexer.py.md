...
7. Для каждого найденного файла вычисляется относительный путь, который преобразуется в имя модуля, Используетсяое в документации Sphinx.
...
- `module_name` (str): Имя модуля, Используетсяое в `index.rst` (путь к файлу с заменой разделителей на точки).
