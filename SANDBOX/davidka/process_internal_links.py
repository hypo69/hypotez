...
class Config:
    ...
    WORK_DIR:Path = Path(STORAGE / 'data_by_supplier')
...
if __name__ == '__main__':
    ...
    try:
        ...
        logger.info(f"Поиск файлов поставщиков в: {Config.WORK_DIR}")
        suppliers_dirs_list = get_directory_names(Config.WORK_DIR) # <- Список директорий поставщиков

        if not suppliers_dirs_list:
            logger.warning(f'Не найдено директорий поставщиков в {Config.WORK_DIR}')
            sys.exit(0) 
        else:
        ...
        while True:
            suppliers_dirs_list = get_directory_names(Config.WORK_DIR) 
            random.shuffle(suppliers_dirs_list)
            for supplier_dir_name in suppliers_dirs_list:  
                supplier_dir_path:str = Config.WORK_DIR / supplier_dir_name  # <- Вот поставщик
                ...
                for supplier_file_name in supplier_file_names:
                    if supplier_file_name == Config.processed_internal_links_file_name:  # <- В этом файле находится лог обработанных ссылок. Пропускаем
                        continue
