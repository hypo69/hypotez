...
2. **Пример содержимого файла `settings.json`**:
   ```json
   {
     "google_drive": "H:\\My Drive\\hypo69",  // Путь к папке Google Drive, Используетсяой для хранения данных.
...
3. **Описание полей**:
   ...
   - **mode**: Укажите режим работы приложения. Используй `debug` для тестирования и `production` для развертывания.
