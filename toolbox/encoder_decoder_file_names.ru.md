...
2. [Описание функциональности](#описание-функциональности)
   - [Методы](#методы)
     - [`encode`](#encode)
     - [`decode`](#decode)
...
3. [Пример использования](#пример-использования)
   ...
   - [Кодирование пути](#кодирование-пути)
   - [Декодирование пути](#декодирование-пути)
...
**Параметры**:
- `file_path` (str): Полный путь к файлу, который необходимо закодировать.

**Возвращает**:
- `str`: Короткий уникальный идентификатор, соответствующий закодированному пути файла.

**Как работает функция**:
- Функция принимает полный путь к файлу в качестве входного параметра.
- Генерирует короткий уникальный идентификатор (8 символов) на основе MD5-хэша пути файла.
- Создает запись в SQLite базе данных, связывающую сгенерированный идентификатор с исходным путем файла.
- Возвращает уникальный идентификатор.
...
print(f'Кодированный путь: {encoded_path}')
```

#### `decode`

```python
def decode(short_id: str) -> Optional[str]:
    """
...
**Параметры**:
- `short_id` (str): Короткий идентификатор, который необходимо декодировать.

**Возвращает**:
- `Optional[str]`: Полный путь файла, соответствующий переданному идентификатору. Если идентификатор не найден в базе данных, возвращает `None`.

**Как работает функция**:
- Функция принимает короткий идентификатор в качестве входного параметра.
- Ищет соответствующий путь файла в SQLite базе данных, используя переданный идентификатор.
- Если идентификатор найден, функция возвращает полный путь файла.
...
**Назначение**: Очищает все записи (соответствия между идентификаторами и путями файлов) из базы данных.
...
**Как работает функция**:
- Функция удаляет все записи из таблицы `path_mapping` в SQLite базе данных.
- После выполнения этой функции база данных становится пустой, и все ранее закодированные пути необходимо кодировать заново.
