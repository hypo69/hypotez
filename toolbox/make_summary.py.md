...
-   `_make_summary(src_dir: Path, summary_file: Path, lang: str = 'en') -> bool`:
    -   Аргументы:
        ...
        -   `lang` (str): Язык фильтрации файлов (`'ru'` или `'en'`). По умолчанию `'en'`.
    -   Возвратое значение: `bool` (True в случае успеха, False в случае ошибки).
...
-   `prepare_summary_path(src_dir: Path, file_name: str = 'SUMMARY.md') -> Path`:
    ...
    -   Возвратое значение: `Path` (новый путь к файлу).
...
-   `args`: Объект, Возвратый `parser.parse_args()`, содержащий значения аргументов командной строки.
