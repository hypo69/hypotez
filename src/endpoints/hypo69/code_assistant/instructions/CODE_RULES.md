Ок, даю  **полный текст `CODE_RULES.md` версии 0.3.0** , уже с:

* ✅ Проект: **AiStros**
* ✅ Лицензия: **CC BY-NC-SA 4.0** (некоммерческое, с атрибуцией и share-alike)
* ✅ Версия: **0.3.0 (WordPress Edition)**
* ✅ Явным требованием: **модель должна прочитать `./gemini/DEPLOYMENT-WORKFLOW.md` перед работой**
* ✅ Без ````rst` блоков

Можешь просто сохранить это как `CODE_RULES.md` в репозитории.
...
Этот документ определяет **единые стандарты кодирования и документирования** для всех модулей экосистемы **AiStros** (включая подпроекты вроде `hypotez`):

- Python-модули и MCP-серверы;
- WordPress-плагины и темы;
- PowerShell-скрипты и DevOps-утилиты;
- HTML/CSS/JS-фронтенд;
- документацию и вспомогательные скрипты.
...
1. **Прочитать файл `./gemini/DEPLOYMENT-WORKFLOW.md` полностью.**  
   В нём описано:
