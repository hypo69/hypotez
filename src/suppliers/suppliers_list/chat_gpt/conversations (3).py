## \file /src/suppliers/chat_gpt/conversations (3).py
# -*- coding: utf-8 -*-
...
from src.webdriver.selenium.driver import Driver, Chrome, Firefox
...
class GPT_Traigner:
    """ Взаимодействие с chat gpt через веб интерфейс """
    driver: Driver

    def __init__(self):
        """"""
        self.driver = Driver(Chrome)
        self.spread_sheet = SpreadSheet(driver=Driver(Firefox))
    ...
    async def catch_current_chat(self, num_of_chats: int = 10):
        ...
        self.save_conversations_to_jsonl(data, gs.path.google_drive / 'chat_gpt' / 'current_chat' / f'_chat.{gs.now}.json')

async def main():
    parser = argparse.ArgumentParser(description="CLI for GPT Trainer")
    parser.add_argument("num_of_chats", type=int, help="Number of chats to catch")
    ...
    while True:
        await trainer.catch_current_chat(args.num_of_chats)
        await asyncio.sleep(1)  # Wait for a moment before next input
