## \file /src/suppliers/aliexpress/campaign/prepare_campaigns (4).py
# -*- coding: utf-8 -*-
...
locales_list:list[dict] = j_loads(gs.path.src / 'suppliers' / 'aliexpress' / 'utils' / 'locales.json').locales
...
def process_campaign(
    ...
    """
    if language is None or currency is None:
        # Process all locales if language or currency is not provided
        locales_list:list[dict] = locales_list
    else:
        locales_list = [(language, currency)]

    for _l in locales_list:
...
def process_all_campaigns(language: Optional[str] = None, currency: Optional[str] = None) -> None:
    ...
    if language is None or currency is None:
        # Process all locales if language or currency is not provided
        locales_to_process = locales.items()
    else:
