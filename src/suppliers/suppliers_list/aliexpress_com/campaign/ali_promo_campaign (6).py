## \file /src/suppliers/aliexpress/campaign/ali_promo_campaign (6).py
# -*- coding: utf-8 -*-
...
class AliPromoCampaign:
    ...
    campaign_ai_file_name: str = None
    locale: str = None  # <- EN_USD
    ...
    def __init__(
        ...
        self.campaign = j_loads_ns(campaign_file_path, exc_info=False) # <- файла может не быть, если я создаю новую рекламную камапнию - файл будет создан ИИ
        if not self.campaign:
            logger.warning(f"Campaign file not found at {campaign_file_path=}\nStart as new \n (Start build JSON file, categories, products etc.)")
            ...
            """
            self.process_new_campaign(campaign_name = campaign_name, language = language, currency = currency)  # <- создание новой рекламной кампании
            return
        ...
        TODO:
            ...
            Сейчас я каждый запуск создаю новый файл. Это неплохо - модель каждый раз генерирует новые ответы
        """
    ...
    def process_campaign(self):
        ...
        """
        ...
        for category_name in vars(self.campaign.category).values():
        ...
        def _process_category(category_name: str, attempts: int = 5):
            ...
            def get_response(_attempts: int = 5):
                try:
                    ...
                    time.sleep(
                        15
                    )  # GoolgeGenerativeAI позволяет max 5 запросов в минуту
                    if _attempts > 0:
            ...
            try:
                ...
                if hasattr(self.campaign.category, category_name):
                    ...
                    setattr(
                        self.campaign.category, category_name, SimpleNamespace(**res)
                    )
            except Exception as ex:
                logger.error(
                    f"Error updating campaign for {category_name=}: ",
                    ex,
                    exc_info=False,
                )
    ...
    def process_new_campaign(self, campaign_name: str, language: Optional[str] = None, currency: Optional[str] = None):
        ...
        ...
        if self.language:
            locale = {self.language:self.currency}
        else:
            locale =  {
                  "EN": "USD",
                  "HE": "ILS",
    ...
    def set_categories_from_directories(self):
        ...
        for  category_name in categories:
            setattr(self.campaign,'category',SimpleNamespace(category_name=category_name, title="", description=""))
