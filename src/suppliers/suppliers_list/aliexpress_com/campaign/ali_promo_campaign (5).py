## \file /src/suppliers/aliexpress/campaign/ali_promo_campaign (5).py
# -*- coding: utf-8 -*-
...
class AliPromoCampaign:
    ...
    def process_campaign(self):
        ...
        """
        ...
        if not self.set_categories_from_directories():
            logger.error(f"Ошибка получения названий категорий")
            ...
            return
        for category in vars(self.campaign.category).values():
            self.process_category_products(category.category_name)
            self.process_llm_category(category.category_name)
    ...
    def process_campaign_category(self, category_name: str) -> list[SimpleNamespace] | None:
        ...
        products = self.process_category_products(category_name = category_name)
