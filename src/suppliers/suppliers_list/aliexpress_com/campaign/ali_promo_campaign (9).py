## \file /src/suppliers/aliexpress/campaign/ali_promo_campaign (9).py
# -*- coding: utf-8 -*-
...
class AliPromoCampaign:
        ...
        def _process_category(category_name: str, attempts: int = 5):
            ...
            def get_response(_attempts: int = 5):
                ...
                try:
                    response = model.ask(
                        prompt
                    )  # The model is expected to return a JSON-formatted dictionary
                    return response
                except Exception as ex:
                    logger.error("Google GoolgeGenerativeAI error:", ex, exc_info=False)
                    time.sleep(
                        15
                    )  # GoolgeGenerativeAI allows max 5 requests per minute
                    if _attempts > 0:
                        get_response(_attempts - 1)
                    return
    ...
    def process_category_products(
        ...
        if not affiliated_products:
            logger.error(f"No affiliated products found.", exc_info=False)
            ...
            return
