...
from selenium.webtab.common.action_chains import ActionChains
from selenium.webtab.common.by import By
from selenium.webtab.common.keys import Keys
from selenium.webtab.remote.webelement import WebElement
from selenium.webtab.support import expected_conditions as EC
from selenium.webtab.support.ui import WebDriverWait
import header
...
class ExecuteLocator:
    ...
    def __post_init__(self):
        if self.tab:
            self.actions = ActionChains(self.tab)
        ...
        try:
            if timeout == 0:
                web_elements = await asyncio.to_thread(
                    tab.find, locator.by, locator.selector
                )
            else:
                condition = (
            ...
            if event == "click()":
                try:
                    ...
                    if locator.mandatory:
                        ...
                        # try:
                        #     self.tab.execute_script("arguments[0].click();", webelement)
                        #     continue
                        # except Exception as ex:
