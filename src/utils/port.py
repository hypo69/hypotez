                ...
                try:
                    if isinstance(item, str):
                        ...
                        if all(isinstance(p, int) for p in item):
                             ...
                             if not (0 <= min_port <= 65535 and 0 <= max_port <= 65535):
                                 logger.warning(f'Предупреждение: Диапазон портов вне допустимого диапазона (0-65535) в элементе {item}. Пропуск.')
                                 # Вместо raise в try-except, просто Пропуск этот элемент.
                                 continue
                             if min_port >= max_port:
                                logger.warning(f'Предупреждение: Некорректный диапазон [min, max] (min >= max) в элементе {item}. Пропуск.')
                                # Вместо raise в try-except, просто Пропуск этот элемент.
                                continue
                        else:
                             # Если элементы списка не являются числами.
                             logger.warning(f'Предупреждение: Некорректный формат диапазона [min, max] (не целые числа) в элементе {item}. Пропуск.')
                             continue # Пропуск этот элемент и переходим к следующему
                    else:
                        # Если элемент списка имеет некорректный тип или формат.
                        logger.warning(f'Предупреждение: Некорректный тип или формат элемента диапазона в списке: {item}. Пропуск.')
                        continue # Пропуск этот элемент и переходим к следующему
                    ...
                    logger.warning(f'Ошибка при обработке элемента диапазона {item}: {e}. Пропуск.')
