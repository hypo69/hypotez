## \file /src/gui/openai_trаigner/browser (3).py
# -*- coding: utf-8 -*-
...
class FramelessWindow(QMainWindow):
    def __init__(self):
        ...
        model_menu = QMenu("Выбрать модель", self)
        chatgpt_action = QAction("ChatGPT", self)
        chatgpt_action.triggered.connect(lambda: self.load_url("https://chatgpt.com"))
        gemini_action = QAction("Gemini", self)
        gemini_action.triggered.connect(lambda: self.load_url("https://gemini.google.com/"))  # Укажите правильный URL
        claude_action = QAction("Claude", self)
        claude_action.triggered.connect(lambda: self.load_url("https://claude.ai/"))  # Укажите правильный URL

        model_menu.addAction(chatgpt_action)
        model_menu.addAction(gemini_action)
    ...
    # Метод для загрузки URL в браузер
